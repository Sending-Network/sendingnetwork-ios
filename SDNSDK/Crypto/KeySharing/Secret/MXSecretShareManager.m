                        ...
                        failure:(void (^)(NSError *error))failure
{
    ...
    return [_crypto.sdnRestClient sendToDevice:payload success:success failure:failure];
}
...
- (void)handleSecretRequest2:(MXSecretShareRequest*)request
{
    ...
    // https://github.com/Sending-Network/SendingnNetwork-ios/issues/3023
...
- (void)shareSecret:(NSString*)secret toRequest:(MXSecretShareRequest*)request
{
    ...
    [_crypto ensureOlmSessionsForDevices:userDevice force:NO success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results) {
        ...
        MXToDevicePayload *payload = [[MXToDevicePayload alloc] initWithEventType:kMXEventTypeStringRoomEncrypted
                                                                       contentMap:contentMap];
        [self.crypto.sdnRestClient sendToDevice:payload success:nil failure:^(NSError *error) {
