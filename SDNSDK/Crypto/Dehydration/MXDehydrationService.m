...
NSString *const MXDehydrationAlgorithm = @"org.sdn.msc2697.v1.olm.libolm_pickle";
NSString *const MXDehydrationServiceErrorDomain = @"org.sdn.sdk.dehydration.service";
...
- (void)dehydrateDeviceWithSDNRestClient:(MXRestClient*)restClient
                                    ...
                                    failure:(void (^)(NSError *error))failure;
{
    ...
    [restClient setDehydratedDevice:dehydratedDevice withDisplayName:@"Backup device" success:^(NSString *deviceId) {
        ...
        [crossSigning signObject:deviceInfo.signalableJSONDictionary withKeyType:MXCrossSigningKeyType.selfSigning success:^(NSDictionary *signedObject) {
            ...
            [self uploadDeviceInfo:deviceInfo forAccount:account withSDNRestClient:restClient success:success failure:failure];
...
- (void)rehydrateDeviceWithSDNRestClient:(MXRestClient*)restClient
                             dehydrationKey:(NSData*)dehydrationKey
                                    success:(void (^)(NSString * deviceId))success
                                    failure:(void (^)(NSError *error))failure;
{
...
- (void)uploadDeviceInfo:(MXDeviceInfo*)deviceInfo
    ...
    withSDNRestClient:(MXRestClient*)restClient
                 success:(void (^)(NSString *deviceId))success
                 failure:(void (^)(NSError *error))failure
{
