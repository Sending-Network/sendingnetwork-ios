...
///  Used to know current node capabilities as per [sdn.org specifications](https://sdn.org/docs/spec/client_server/r0.6.1#get-sdn-client-r0-capabilities)
///  and [MSC3244](https://github.com/sdn-org/sdn-doc/pull/3244)
@objcMembers
public class MXNodeCapabilitiesService: NSObject {
    
    // MARK: - Members
    
    private let session: MXSession
    private let mapper = MXRoomCapabilityTypeMapper()
    private var capabilities: MXNodeCapabilities?
    private var currentRequest: MXHTTPOperation?
    
    // MARK: - Properties
    
    /// True if service succesfully read capabilities from the node. False otherwise.
    ...
    public func update(completion: (() -> Void)? = nil) {
        guard currentRequest == nil else {
            MXLog.debug("[MXNodeCapabilitiesService] update: aborted")
            return
        }
        
        self.currentRequest = session.sdnRestClient.nodeCapabilities { [weak self] response in
            ...
            case .failure(let error):
                MXLog.error("[MXNodeCapabilitiesService] update: failed", context: error)
            }
    ...
    /// Check if a feature is supported by the node.
    /// - Parameters:
    ...
    /// Check if a feature is supported by the node and for a given room version
    /// - Parameters:
    ///   - feature: Type of the room capability
...
extension MXNodeCapabilitiesService {
    
    /// Update the instance with given server capabilities (only for test purpose)
    /// - Parameters:
    ///   - capabilities: new node capabilities
    public func update(with capabilities: MXNodeCapabilities?) {
        self.capabilities = capabilities
    }
