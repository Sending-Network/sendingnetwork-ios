...
- (id)initWithNode:(NSString *)nodeURL
{
    self = [super init];
    if (self)
    {
...
- (NSString*)urlOfContent:(NSString*)mxContentURI
{
    ...
    if ([mxContentURI hasPrefix:kMXContentUriScheme])
    {
        ...
        {
            mxMediaPrefix = [NSString stringWithFormat:@"%@/%@/download/", _nodeURL, kMXContentPrefixPath];
        }
                        ...
                        withMethod:(MXThumbnailingMethod)thumbnailingMethod
{
    ...
    if ([mxContentURI hasPrefix:kMXContentUriScheme])
    {
        ...
        {
            mxThumbnailPrefix = [NSString stringWithFormat:@"%@/%@/thumbnail/", _nodeURL, kMXContentPrefixPath];
        }
...
{
    return [NSString stringWithFormat:@"%@/%@/identicon/%@", _nodeURL, kMXContentPrefixPath, [identiconString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]]];
}


#pragma mark - Media Download

+ (NSString*)downloadIdForSDNContentURI:(NSString*)mxContentURI
                                  inFolder:(NSString*)folder
{
    // Return the unique output file path built from the mxc uri and the potential folder (no type is required here)
    return [MXMediaManager cachePathForSDNContentURI:mxContentURI andType:nil inFolder:folder];
}

+ (NSString*)thumbnailDownloadIdForSDNContentURI:(NSString*)mxContentURI
                                           inFolder:(NSString*)folder
                                      toFitViewSize:(CGSize)viewSize
                                         withMethod:(MXThumbnailingMethod)thumbnailingMethod
{
    // Return the unique output file path built from the mxc uri and the potential folder (no type is required here)
    return [MXMediaManager thumbnailCachePathForSDNContentURI:mxContentURI andType:nil inFolder:folder toFitViewSize:viewSize withMethod:thumbnailingMethod];
}

- (MXMediaLoader*)downloadMediaFromSDNContentURI:(NSString *)mxContentURI
                                            ...
                                            failure:(void (^)(NSError *error))failure
{
    ...
    if (!mediaURL)
    {
        MXLogDebug(@"[MXMediaManager] downloadMediaFromSDNContentURI: invalid media content URI");
        if (failure) failure(nil);
        return nil;
    }
    
    // Build the outpout file path from mxContentURI, and other inputs.
    NSString *filePath = [MXMediaManager cachePathForSDNContentURI:mxContentURI andType:mimeType inFolder:folder];
    
    // Build the download id from mxContentURI.
    NSString *downloadId = [MXMediaManager downloadIdForSDNContentURI:mxContentURI inFolder:folder];
...
- (MXMediaLoader*)downloadMediaFromSDNContentURI:(NSString *)mxContentURI
                                           ...
                                           inFolder:(NSString *)folder
{
    return [self downloadMediaFromSDNContentURI:mxContentURI withType:mimeType inFolder:folder success:nil failure:nil];
}

- (MXMediaLoader*)downloadThumbnailFromSDNContentURI:(NSString *)mxContentURI
                                               withType:(NSString *)mimeType
                                               inFolder:(NSString *)folder
                                          toFitViewSize:(CGSize)viewSize
                                             withMethod:(MXThumbnailingMethod)thumbnailingMethod
                                                ...
                                                failure:(void (^)(NSError *error))failure
{
    ...
    if (!mediaURL)
    {
        MXLogDebug(@"[MXMediaManager] downloadThumbnailFromSDNContentURI: invalid media content URI");
        if (failure) failure(nil);
        return nil;
    }
    
    // Build the outpout file path from mxContentURI, and other inputs.
    NSString *filePath = [MXMediaManager thumbnailCachePathForSDNContentURI:mxContentURI andType:mimeType inFolder:folder toFitViewSize:viewSize withMethod:thumbnailingMethod];
    
    // Build the download id from mxContentURI.
    NSString *downloadId = [MXMediaManager thumbnailDownloadIdForSDNContentURI:mxContentURI inFolder:folder toFitViewSize:viewSize withMethod:thumbnailingMethod];
                        ...
                        failure:(void (^)(NSError *error))failure
{
    ...
    if (mediaLoader)
    {
        // This mediaLoader has been created for the same sdn content uri, and cache folder.
        if (success || failure)
        {
...
- (MXMediaLoader*)downloadEncryptedMediaFromSDNContentFile:(MXEncryptedContentFile *)encryptedContentFile
                                                      ...
                                                      failure:(void (^)(NSError *error))failure
{
    ...
    if (!downloadMediaURL)
    {
        MXLogDebug(@"[MXMediaManager] downloadEncryptedMediaFromSDNContentFile: invalid media content URI");
        if (failure) failure(nil);
        return nil;
    }
    
    // Build the outpout file path from mxContentURI, and other inputs.
    NSString *filePath = [MXMediaManager cachePathForSDNContentURI:mxContentURI
                                                              andType:mimeType
                                                             inFolder:folder];
    
    // Build the download id from mxContentURI.
    NSString *downloadId = [MXMediaManager downloadIdForSDNContentURI:mxContentURI
                                                                inFolder:folder];
...
- (MXMediaLoader*)downloadEncryptedMediaFromSDNContentFile:(MXEncryptedContentFile *)encryptedContentFile
                                                     ...
                                                     inFolder:(NSString *)folder
{
    return [self downloadEncryptedMediaFromSDNContentFile:encryptedContentFile
                                                    mimeType:mimeType
                                                    inFolder:folder
                                                     success:nil
                                                     failure:nil];
...
+ (MXMediaLoader*)prepareUploaderWithSDNSession:(MXSession*)mxSession
                                           ...
                                           andRange:(CGFloat)range
{
    if (mxSession)
    {
        ...
        MXMediaLoader *mediaLoader = [[MXMediaLoader alloc] initForUploadWithSDNSession:mxSession initialRange:initialRange andRange:range];
        // Report this loader
        if (!uploadTableById)
        {
...
+ (NSString*)cachePathForSDNContentURI:(NSString*)mxContentURI andType:(NSString *)mimeType inFolder:(NSString*)folder
{
    ...
    {
        MXLogDebug(@"[MXMediaManager] cachePathForSDNContentURI: invalid media content URI");
        return nil;
    }
...
+ (NSString*)thumbnailCachePathForSDNContentURI:(NSString*)mxContentURI
                                        ...
                                        withMethod:(MXThumbnailingMethod)thumbnailingMethod
{
    ...
    if (![mxContentURI hasPrefix:kMXContentUriScheme])
    {
        MXLogDebug(@"[MXMediaManager] thumbnailCachePathForSDNContentURI: invalid media content URI");
        return nil;
    }
