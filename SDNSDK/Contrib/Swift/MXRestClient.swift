...
public extension MXRestClient {
     ...
     */
    @nonobjc required convenience init(node: URL, unrecognizedCertificateHandler handler: MXHTTPClientOnUnrecognizedCertificate?) {
        self.init(__node: node.absoluteString, andOnUnrecognizedCertificateBlock: handler)
    }
    ...
    /**
     ...
     As described in [the specification](http://sdn.org/docs/spec/client_server/r0.2.0.html#client-authentication),
    ...
    /**
     ...
     - parameters:
         ...
         - username: the user id (ex: "@bob:sdn.org") or the user id localpart (ex: "bob") of the user to register. Can be nil.
    ...
    /**
     ...
     - parameters:
         ...
         - username: the user id (ex: "@bob:sdn.org") or the user id localpart (ex: "bob") of the user to authenticate.
         - password: the user's password.
    ...
    /**
     ...
     - parameters:
        ...
        - append: If `true`, the node should add another pusher with the given pushkey and App ID in addition to any others with different user IDs.
    ...
    /**
     ...
     - parameters:
        ...
        - roomId: the id of the room.
        - allowedParentIds: Optional: list of allowedParentIds (required only for `restricted` join rule as per [MSC3083](https://github.com/sdn-org/sdn-doc/pull/3083) )
        - completion: A block object called when the operation completes.
        - response: Indicates whether the operation was successful.
    ...
    /**
     ...
     - parameters:
        - roomId: the id of the room.
        - completion: A block object called when the operation completes. It provides the room enhanced join rule as per [MSC3083](https://github.com/sdn-org/sdn-doc/pull/3083.
    ...
    /**
     ...
     - parameters:
        - parameters: The parameters. Refer to the sdn specification for details.
        - completion: A block object called when the operation completes.
        - response: Provides a MXCreateRoomResponse object on success.
    ...
    /**
     ...
     See [the sdn documentation on state events](http://sdn.org/docs/api/client-server/#!/-rooms/get_state_events)
     for more detail.
    ...
    /**
     ...
     See [the sdn documentation](http://sdn.org/docs/api/client-server/#!/-rooms/get_room_sync_data)
     for more detail.
     ...
     - parameters:
        ...
        - completion: A block object called when the operation completes.
        - response: Provides the model created from the node JSON response on success.
    ...
    /**
     ...
     - parameters:
        ...
        - via: servers, that should be tried to request a summary from, if it can't be generated locally. These can be from a sdn URI, sdn.to link or a `m.space.child` event for example.
        - completion: A block object called when the operation completes.
        - response: Provides the model created from the node JSON response on success.
    ...
    /**
     ...
     - parameters:
        ...
        - bind: whether the node should also bind this third party identifier to the account's SDN ID with the identity server.
        - completion: A block object called when the operation completes.
        - response:  Indicates whether the operation was successful.
    ...
    */
    ...
    /**
     ...
     - parameters:
        - server: (optional) the remote server to query for the room list. If nil, get the user node's public room list.
    ...
    /**
     ...
     - parameters:
        - parameters: the search parameters as defined by the SDN search spec (http://sdn.org/docs/api/client-server/#!/Search/post_search ).
        - nextBatch: the token to pass for doing pagination from a previous response.
        - completion: A block object called when the operation completes.
        - response: Provides the search results on success.
    ...
    ///   - completion: A closure called when the operation completes.
    /// - Returns: a `MXHTTPOperation` instance.
    @nonobjc @discardableResult func nodeCapabilities(completion: @escaping (_ response: MXResponse<MXNodeCapabilities>) -> Void) -> MXHTTPOperation {
        return __nodeCapabilities(success: currySuccess(completion), failure: curryFailure(completion))
    }
    ...
    /// Get the supported versions of the node
    /// - Parameters:
    ///   - completion: A closure called when the operation completes.
    /// - Returns: a `MXHTTPOperation` instance.
    @nonobjc @discardableResult func supportedSDNVersions(completion: @escaping (_ response: MXResponse<MXSDNVersions>) -> Void) -> MXHTTPOperation {
        return __supportedSDNVersions(currySuccess(completion), failure: curryFailure(completion))
    }
}
