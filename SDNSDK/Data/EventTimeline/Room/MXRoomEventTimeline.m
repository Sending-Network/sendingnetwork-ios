...
@interface MXRoomEventTimeline ()
{
    ...
    BOOL hasReachedNodeForwardsPaginationEnd;
...
- (instancetype)initWithRoom:(MXRoom*)theRoom andInitialEventId:(NSString*)initialEventId
{
    ...
    if (initialEventId)
    {
        ...
        [memoryStore openWithCredentials:theRoom.mxSession.sdnRestClient.credentials onComplete:nil failure:nil];
...
- (instancetype)initWithRoom:(MXRoom*)theRoom initialEventId:(NSString*)initialEventId andStore:(id<MXStore>)theStore
{
    if (self = [self init])
    {
        ...
        _state = [[MXRoomState alloc] initWithRoomId:room.roomId andSDNSession:room.mxSession andDirection:YES];
...
- (BOOL)canPaginate:(MXTimelineDirection)direction
{
    ...
    if (direction == MXTimelineDirectionBackwards)
    {
        ...
        canPaginate = (0 < storeMessagesEnumerator.remaining)
            || ![store hasReachedNodePaginationEndForRoom:_state.roomId];
    }
    else
    {
        if (_isLiveTimeline)
        {
            // SDN is not yet able to guess the future
            canPaginate = NO;
        }
        else
        {
            canPaginate = !hasReachedNodeForwardsPaginationEnd;
        }
    }
...
- (MXHTTPOperation *)resetPaginationAroundInitialEventWithLimit:(NSUInteger)limit success:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    ...
    hasReachedNodeForwardsPaginationEnd = NO;

    // Get the context around the initial event
    MXWeakify(self);
    return [room.mxSession.sdnRestClient contextOfEvent:_initialEventId inRoom:room.roomId limit:limit filter:_roomEventFilter success:^(MXEventContext *eventContext) {
...
- (MXHTTPOperation *)paginate:(NSUInteger)numItems direction:(MXTimelineDirection)direction onlyFromStore:(BOOL)onlyFromStore complete:(void (^)(void))complete failure:(void (^)(NSError *))failure
{
    ...
    [self paginateFromStore:numItems direction:direction onComplete:^(NSArray<MXEvent *> *eventsFromStore) {
        ...
        if (direction == MXTimelineDirectionBackwards)
        {
            ...
            if (remainingNumItems <= 0 || YES == [self->store hasReachedNodePaginationEndForRoom:self.state.roomId])
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    // Nothing more to do
        ...
        if (direction == MXTimelineDirectionForwards && YES == self->hasReachedNodeForwardsPaginationEnd)
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                // Nothing more to do
                MXLogDebug(@"[MXRoomEventTimeline] paginate: is done");
        ...
        MXWeakify(self);
        MXHTTPOperation *operation2 = [self->room.mxSession.sdnRestClient messagesForRoom:self.state.roomId from:paginationToken direction:direction limit:remainingNumItems filter:self.roomEventFilter success:^(MXPaginationResponse *paginatedResponse) {
            MXStrongifyAndReturnIfNil(self);
            ...
            if (mxError && [mxError.error isEqualToString:kMXErrorStringInvalidToken])
            {
                ...
                if (direction == MXTimelineDirectionBackwards)
                {
                    [self->store storeHasReachedNodePaginationEndForRoom:self->_state.roomId andValue:YES];
                }
                else
                {
                    self->hasReachedNodeForwardsPaginationEnd = YES;
                }
...
#pragma mark - Node responses handling
- (void)handleJoinedRoomSync:(MXRoomSync *)roomSync onComplete:(void (^)(void))onComplete
{
    ...
    [self decryptEvents:roomSync.timeline.events ifNewerThanTimestamp:timestamp onComplete:^{
        ...
        if (isRoomInitialSync)
        {
            ...
            {
                [self->store storeHasReachedNodePaginationEndForRoom:self.state.roomId andValue:YES];
            }
        }
        else
        {
...
- (void)handlePaginationResponse:(MXPaginationResponse*)paginatedResponse direction:(MXTimelineDirection)direction onComplete:(void (^)(void))onComplete
{
    ...
    // End token might be ommited when end of the timeline is reached: https://github.com/sdn-org/synapse/pull/12903
    if (paginatedResponse.chunk.count == 0 && (paginatedResponse.end == nil || [paginatedResponse.start isEqualToString:paginatedResponse.end]))
    {
        ...
        if (direction == MXTimelineDirectionBackwards)
        {
            [store storeHasReachedNodePaginationEndForRoom:_state.roomId andValue:YES];
        }
        else
        {
            hasReachedNodeForwardsPaginationEnd = YES;
        }
    }
...
- (void)addEvent:(MXEvent*)event direction:(MXTimelineDirection)direction fromStore:(BOOL)fromStore isRoomInitialSync:(BOOL)isRoomInitialSync
{
    ...
    if (_isLiveTimeline && direction == MXTimelineDirectionForwards)
    {
        ...
        // as the node provides sanitised data in this situation
...
- (void)handleRedaction:(MXEvent*)redactionEvent
{
    ...
    // TODO: This is no more implemented (see https://github.com/Sending-Network/SendingnNetwork-ios/issues/443).
    ...
    {
        MXLogDebug(@"[MXRoomEventTimeline] handleRedaction: the redacted event is unknown. Fetch it from the node");
...
- (void)handleStateEvents:(NSArray<MXEvent *> *)stateEvents direction:(MXTimelineDirection)direction
{
    ...
    else
    {
        ...
        if (!room.mxSession.syncWithLazyLoadOfRoomMembers && ![store hasLoadedAllRoomMembersForRoom:room.roomId])
        {
            ...
            // XXX: Optimisation removed because of https://github.com/Sending-Network/sendingnetwork-ios/issues/3807
