...

{
    ...
    operation = [crypto ensureOlmSessionsForDevices:devicesByUser force:NO success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results) {
        ...
        if (haveTargets)
        {
            ...
            MXHTTPOperation *operation2 = [self->crypto.sdnRestClient sendToDevice:payload success:^{
    ...
    operation = [crypto ensureOlmSessionsForDevices:@{
                                            ...
                                            success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results)
                 {
                     ...
                     MXToDevicePayload *toDevicePayload = [[MXToDevicePayload alloc] initWithEventType:kMXEventTypeStringRoomEncrypted
                                                                                            contentMap:contentMap];
                     MXHTTPOperation *operation2 = [self->crypto.sdnRestClient sendToDevice:toDevicePayload success:success failure:failure];
                     [operation mutateTo:operation2];
