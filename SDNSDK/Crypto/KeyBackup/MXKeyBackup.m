...
- (void)checkAndStartKeyBackup
{
    ...
    self.state = MXKeyBackupStateCheckingBackUpOnNode;
...
- (void)maybeSendKeyBackup
{
    ...
    else
    {
        ...
        // If not already done, check for a valid backup version on the node.
...
- (MXHTTPOperation *)versionFromCryptoQueue:(NSString *)version success:(void (^)(MXKeyBackupVersion * _Nullable))success failure:(void (^)(NSError * _Nonnull))failure
{
    return [self.restClient keyBackupVersion:version success:success failure:^(NSError *error) {
        ...
        // Workaround because the node currently returns  M_NOT_FOUND when there is
        // no key backup
        MXError *mxError = [[MXError alloc] initWithNSError:error];
                                   ...
                                   failure:(nullable void (^)(NSError *error))failure
{
    ...
    dispatch_async(cryptoQueue, ^{
        ...
        MXHTTPOperation *operation2 = [self.restClient deleteKeyBackupVersion:version success:^{
            ...
            // Do not stay in MXKeyBackupStateUnknown but check what is available on the node
            if (self.state == MXKeyBackupStateUnknown)
            {
                [self checkAndStartKeyBackup];
            }
...
- (MXHTTPOperation*)forceRefresh:(nullable void (^)(BOOL valid))success
                         failure:(nullable void (^)(NSError *error))failure
{
    if (_state == MXKeyBackupStateUnknown || _state == MXKeyBackupStateCheckingBackUpOnNode)
    {
    ...
    return [self versionFromCryptoQueue:nil success:^(MXKeyBackupVersion * _Nullable serverKeyBackupVersion) {
        ...
        dispatch_async(self->cryptoQueue, ^{
            ...
            {
                MXLogDebug(@"[MXKeyBackup] forceRefresh: New version detected on the node. New version: %@. Old version: %@", serverKeyBackupVersion.version, self.keyBackupVersion.version);
                usingLastVersion = NO;
...
- (BOOL)canBeRefreshed
{
    return _state != MXKeyBackupStateUnknown && _state != MXKeyBackupStateCheckingBackUpOnNode;
}
...
- (NSString *)descriptionForState:(MXKeyBackupState)state
{
    switch (state) {
        case MXKeyBackupStateUnknown:
            return @"Unknown";
        case MXKeyBackupStateCheckingBackUpOnNode:
            return @"CheckingBackUpOnNode";
