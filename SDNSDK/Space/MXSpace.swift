...
extension MXSpaceError: CustomNSError {
    public static let errorDomain = "org.sdn.sdk.space"
...
/// A SDN space enables to collect rooms together into groups. Such collections of rooms are referred as "spaces" (see https://github.com/sdn-org/sdn-doc/blob/matthew/msc1772/proposals/1772-groups-as-rooms.md).
@objcMembers
public class MXSpace: NSObject {
    ...
    public init(roomId: String, session: MXSession) {
        ...
        self.processingQueue = DispatchQueue(label: "org.sdn.sdk.MXSpace.processingQueue", attributes: .concurrent)
    ...
    ///   If nil value is set current node will be used as via server.
    ...
    public func addChild(roomId: String,
        ...
        if let viaServers = viaServers {
            ...
            // If viaServers is nil use the current node as via server
            guard let nodeName = self.session.credentials.nodeName() else {
                completion(.failure(MXSpaceError.nodeNameNotFound))
                return nil
            }
            finalViaServers = [nodeName]
        }
