...
/**
 `MXRestClient` makes requests to SDN servers.

 It is the single point to send requests to SDN servers which are:
    - the specified SDN home server
    - the SDN content repository manage by this home server
    - the specified SDN identity server
 */
...
/**
 Credentials for the SDN Client-Server API.
 */
...
/**
 The node URL.
 Shortcut to credentials.node.
 */
@property (nonatomic, readonly) NSString *node;

/**
 The node suffix (for example ":sdn.org"). Available only when credentials have been set.
 */
@property (nonatomic, readonly) NSString *nodeSuffix;
...
/**
 The SDN content repository prefix to use.
...
/**
 Create an instance based on node url.
 ...
 */
-(id)initWithNode:(NSString *)node andOnUnrecognizedCertificateBlock:(MXHTTPClientOnUnrecognizedCertificate)onUnrecognizedCertBlock NS_REFINED_FOR_SWIFT;

/**
 Create an instance based on a sdn user account.
...
/**
 ...
 @return a MXHTTPOperation instance.
 */
- (MXHTTPOperation*)supportedSDNVersions:(void (^)(MXSDNVersions *sdnVersions))success
                                    failure:(void (^)(NSError *error))failure NS_REFINED_FOR_SWIFT;

/**
 Get the wellknwon data of the node.
...
/**
 ...
 As described in http://sdn.org/docs/spec/client_server/r0.2.0.html#client-authentication some registration flows require to
...
/**
 ...
 This method manages the full flow for simple login types and returns the credentials of the newly created sdn user.
 ...
 @param username the user id (ex: "@bob:sdn.org") or the user id localpart (ex: "bob") of the user to register. Can be nil.
...
/**
 ...
 Check MXSDNVersion.doesServerRequireIdentityServerParam to determine if
 the node requires the id_server parameter to be provided.
...
/**
 ...
 This method manages the full flow for simple login types and returns the credentials of the logged sdn user.
 ...
 @param username the user id (ex: "@bob:sdn.org") or the user id localpart (ex: "bob") of the user to register.
 @param password his password.
 @param success A block object called when the operation succeeds. It provides credentials to use to create a MXRestClient.
 @param failure A block object called when the operation fails.
...
/**
 ...
 of the SDN account they are logged into.
...
/**
 ...
 @param append If true, the node should add another pusher with the given pushkey and App ID in addition to any others with different user IDs.
 @param success A block object called when the operation succeeds. It provides credentials to use to create a MXRestClient.
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param joinRule the rule to set.
 @param roomId the id of the room.
 @param allowedParentIds Optional: list of allowedParentIds (required only for `restricted` join rule as per [MSC3083](https://github.com/sdn-org/sdn-doc/pull/3083) )
 @param success A block object called when the operation succeeds.
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param roomId the id of the room.
 @param success A block object called when the operation succeeds. It provides the room enhanced join rule as per [MSC3083](https://github.com/sdn-org/sdn-doc/pull/3083.
 @param failure A block object called when the operation fails.
...
/**
 Set the directory visibility of a room on the current node.
...
/**
 ...
 @param parameters the parameters. Refer to the sdn specification for details.
...
/**
 ...
 home server JSON response. @see http://sdn.org/docs/api/client-server/#!/-rooms/get_state_events
 @param failure A block object called when the operation fails.
...
/**
 ...
 You can check whether a node supports the redaction with relations via
...
/**
 ...
 the node JSON response. @see http://sdn.org/docs/api/client-server/#!/-rooms/get_room_sync_data
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param via servers, that should be tried to request a summary from, if it can't be generated locally. These can be from a sdn URI, sdn.to link or a `m.space.child` event for example.
...
/**
 Link an authenticated 3rd party id to the SDN user.
...
/**
 Add a 3PID to your node account.

 This API does not use an identity server, as the node is expected to
...
/**
 Remove a 3rd party id from the SDN user information.
...
/**
 List all 3PIDs linked to the SDN user account.
...
/**
 Bind a 3PID for discovery onto an identity server via the node.

 The identity server handles 3PID ownership validation and the node records
...
/**
 Unbind a 3PID for discovery on an identity server via the node.

 The node removes its record of the binding to keep an updated record of
...
/**
 Retrieve a SDN User ID linked to a user on the third party service, given a set of user parameters.
...
/**
 ...
 @param mxcContentURI the SDN content URI to scan (in the form of "mxc://...").
...
/**
 Get the TURN server configuration advised by the node.
...
/**
 ...
 @param parameters the search parameters as defined by the SDN search spec (http://sdn.org/docs/api/client-server/#!/Search/post_search ).
 @param nextBatch the token to pass for doing pagination from a previous response.
...
/**
 Search users on node user directory.
...
/**
 Back up a session key to the node.
...
/**
 Back up keys in a room to the node.
...
/**
 Retrieve the backup for a session key from the node.
...
/**
 Retrieve the backup for all keys in a room from the node.
...
/**
 Retrieve all keys backup from the node.
...
/**
 Delete the backup for a session key from the node.
...
/**
 Delete the backup for all keys in a room from the node.
...
/**
 Delete all keys backup from the node.
...
/// @param success A block object called when the operation succeeds. It provides a `MXNodeCapabilities` object.
/// @param failure A block object called when the operation fails.
/// @return a MXHTTPOperation instance.
- (MXHTTPOperation*)nodeCapabilitiesWithSuccess:(void (^)(MXNodeCapabilities *capabilities))success
                                              failure:(void (^)(NSError *error))failure NS_REFINED_FOR_SWIFT;
...
- (MXHTTPOperation *)getSDMLoginRandomAddress:(NSString *)address
