...
/**
 `MXFilterJSONModel` represents a sdn filter model as exchanged through the SDN
 Client-Server API.
 */
...
/**
 Build a SDN filter to retrieve a max given number of messages per room in /sync requests.

 @param messageLimit messageLimit the messages count limit.
 @return the SDN filter.
 */
+ (MXFilterJSONModel*)syncFilterWithMessageLimit:(NSUInteger)messageLimit;

/**
 Build a SDN filter to enable room members lazy loading in /sync requests.
