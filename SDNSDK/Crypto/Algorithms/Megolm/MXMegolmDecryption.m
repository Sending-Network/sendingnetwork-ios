...
@interface MXMegolmDecryption ()
{
    ...
    // senderKey|sessionId to timelines to list of SDNEvents
    ...
    operation = [crypto ensureOlmSessionsForDevices:@{
                                ...
                                success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results)
     {
        ...
        MXLogDebug(@"[MXMegolmDecryption] shareKeysWithDevices: sharing keys for session %@|%@ with devices of user %@", senderKey, sessionId, userId);
         ...
         MXHTTPOperation *operation2 = [self->crypto.sdnRestClient sendToDevice:toDevicePayload
                                                                           success:success
                                                                           failure:failure];
...
- (void)requestKeysForEvent:(MXEvent*)event
{
    ...
    NSString *myUserId = crypto.sdnRestClient.credentials.userId;
