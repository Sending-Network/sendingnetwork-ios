...
@interface MXNoStore () <MXEventsEnumeratorDataSource>
{
    ...
    NSMutableDictionary *hasReachedNodePaginations;
    ...
    // All sdn users known by the user
    // The keys are user ids.
    NSMutableDictionary <NSString*, MXUser*> *users;
    
    // All sdn groups known by the user
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        hasReachedNodePaginations = [NSMutableDictionary dictionary];
...
- (void)deleteRoom:(NSString *)roomId
{
    ...
    if (hasReachedNodePaginations[roomId])
    {
        [hasReachedNodePaginations removeObjectForKey:roomId];
    }
...
- (void)deleteAllData
{
    ...
    [hasReachedNodePaginations removeAllObjects];
...
- (void)storeHasReachedNodePaginationEndForRoom:(NSString*)roomId andValue:(BOOL)value
{
    hasReachedNodePaginations[roomId] = [NSNumber numberWithBool:value];
}

- (BOOL)hasReachedNodePaginationEndForRoom:(NSString*)roomId
{
    BOOL hasReachedNodePaginationEnd = NO;

    NSNumber *hasReachedNodePaginationEndNumber = hasReachedNodePaginations[roomId];
    if (hasReachedNodePaginationEndNumber)
    {
        hasReachedNodePaginationEnd = [hasReachedNodePaginationEndNumber boolValue];
    }

    return hasReachedNodePaginationEnd;
}
...
- (id<MXEventsEnumerator>)messagesEnumeratorForRoom:(NSString *)roomId
{
    ...
    [self storeHasReachedNodePaginationEndForRoom:roomId andValue:NO];
...
- (void)storeNodeWellknown:(nonnull MXWellKnown *)nodeWellknown
{
}
...
- (void)storeNodeCapabilities:(MXCapabilities *)nodeCapabilities
{
}

- (MXSDNVersions *)supportedSDNVersions
{
    return nil;
}
- (void)storeSupportedSDNVersions:(MXSDNVersions *)supportedSDNVersions
{
}
