...
NSString *const MXKeyVerificationErrorDomain = @"org.sdn.sdk.verification";
                     ...
                     failure:(void(^)(NSError *error))failure
{
    [self.crypto downloadKeys:@[userId] forceDownload:YES success:^(MXUsersDevicesMap<MXDeviceInfo *> *usersDevicesInfoMap, NSDictionary<NSString *,MXCrossSigningInfo *> *crossSigningKeysMap) {
        ...
        MXCredentials *myUser = self.crypto.mxSession.sdnRestClient.credentials;
                                         ...
                                         failure:(void(^)(NSError *error))failure
{
    ...
    [self.crypto.sdnRestClient sendToDevice:payload success:^{
...
- (instancetype)initWithCrypto:(MXLegacyCrypto *)crypto
{
    ...
    if (self)
    {
        ...
        statusResolver = [[MXKeyVerificationStatusResolver alloc] initWithManager:self sdnSession:crypto.mxSession];
...
- (void)handleReadyEvent:(MXEvent*)event isToDeviceEvent:(BOOL)isToDeviceEvent
{
    ...
    if (request)
    {
        MXCredentials *myCreds = _crypto.mxSession.sdnRestClient.credentials;
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    MXToDevicePayload *payload = [[MXToDevicePayload alloc] initWithEventType:eventType
                                                                ...
                                                                transactionId:nil
                                                                 addMessageId:NO]; // // Should not add anything for verification events as it would break their signatures
    return [self.crypto.sdnRestClient sendToDevice:payload success:success failure:failure];
}
...
{
    NSString *currentUserId = self.crypto.mxSession.myUserId;
    NSString *currentDeviceId = self.crypto.mxSession.sdnRestClient.credentials.deviceId;
