...
@interface MXLoginFlow : MXJSONModel
    ...
    /**
     ...
     @see http://sdn.org/docs/spec/#password-based and below for the types descriptions
     */
...
@interface MXLoginResponse : MXJSONModel
    ...
    /**
     The home server url (ex: "https://sdn.org").
     */
...
/**
 The node name.
 */
@property (nonatomic) NSString *sdnServerName;
...
/**
 ...
 specified by SDN are listed here as NSUInteger enum in order to ease
...
typedef enum : NSUInteger
{
    ...
    MXPushRuleActionTypeCoalesce,   // At a SDN client level, coalesce action should be treated as a notify action
...
@interface MXPushRulesSet : MXJSONModel
    ...
    /**
     These rules configure notification behaviour for messages from a specific, named SDN user ID. 
     The rule_id of Sender rules is always the SDN user ID of the user whose messages theyt apply to.
     */
...
@interface MXRoomInitialSync : MXJSONModel
    ...
    /**
     The sdn id of the inviter in case of pending invitation.
     */
    ...
    /**
     The presence status of other users (Provided in case of room initial sync @see http://sdn.org/docs/api/client-server/#!/-rooms/get_room_sync_data)).
     */
...
/**
 The count per algorithm as returned by the node: a map (algorithm->count).
 */
...
@interface MXKeysClaimResponse : MXJSONModel
    ...
    /**
     If any remote nodes could not be reached, they are recorded here. 
     The names of the properties are the names of the unreachable servers.

     If the node could be reached, but the user or device was unknown, 
...
/**
 `MXRoomJoinRuleResponse` represents the enhanced join rule response as per [MSC3083](https://github.com/sdn-org/sdn-doc/pull/3083)
 */
...
/**
 `MXNodeCapabilities` the capabilities of the current node
 */
@interface MXNodeCapabilities : MXJSONModel
