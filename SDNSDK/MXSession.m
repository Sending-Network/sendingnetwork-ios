...
NSString *const kMXSessionNoRoomTag = @"m.recent";  // Use the same value as sdn-react-sdk
...
@interface MXSession ()
{
    ...
    /**
     The tool to refresh the node wellknown data.
     */
...
@synthesize sdnRestClient, mediaManager;

- (id)initWithSDNRestClient:(MXRestClient*)mxRestClient
{
    self = [super init];
    if (self)
    {
        sdnRestClient = mxRestClient;
        _threePidAddManager = [[MX3PidAddManager alloc] initWithSDNSession:self];
        mediaManager = [[MXMediaManager alloc] initWithNode:sdnRestClient.node];
        ...
        _notificationCenter = [[MXNotificationCenter alloc] initWithSDNSession:self];
        ...
        _nodeCapabilitiesService = [[MXNodeCapabilitiesService alloc] initWithSession: self];
        [_nodeCapabilitiesService updateWithCompletion:nil];
...
{
    return sdnRestClient.credentials;
}

- (NSString *)myUserId
{
    return sdnRestClient.credentials.userId;
}

- (NSString *)myDeviceId
{
    return sdnRestClient.credentials.deviceId;
}
...
-(void)setStore:(id<MXStore>)store success:(void (^)(void))onStoreDataReady failure:(void (^)(NSError *))failure
{
    ...
    _storeService = [[MXStoreService alloc] initWithStore:store credentials:sdnRestClient.credentials];
    ...
    MXWeakify(self);
    [self.store openWithCredentials:sdnRestClient.credentials onComplete:^{
        MXStrongifyAndReturnIfNil(self);
        
        // Sanity check: The session may be closed before the end of store opening.
        if (!self->sdnRestClient)
        {
            return;
        }

        self.storeService.aggregations = [[MXAggregations alloc] initWithSDNSession:self];
        ...
        [self initializeCryptoWithProgress:^(double progress) {
            ...
            if (self.store.isPermanent && self.isEventStreamInitialised)
            {
                ...
                MXUser *myUser = [self.store userWithUserId:self->sdnRestClient.credentials.userId];
            ...
            else
            {
                ...
                self->_myUser = [[MXMyUser alloc] initWithUserId:self->sdnRestClient.credentials.userId];
                self->_myUser.mxSession = self;
           ...
           storeCompletion:(void (^)(void))storeCompletion
{
    ...
    [self handleCryptoEventsInSyncResponse:syncResponse onComplete:^{
        ...
        dispatch_group_notify(dispatchGroup, dispatch_get_main_queue(), ^{
            ...
            if (!self.nodeCapabilitiesService.isInitialised)
            {
                [self.nodeCapabilitiesService updateWithCompletion:nil];
            }
...
- (void)setIdentityServer:(NSString *)identityServer andAccessToken:(NSString *)accessToken
{
    ...
    sdnRestClient.identityServer = identityServer;

    if (identityServer)
    {
        _identityService = [[MXIdentityService alloc] initWithIdentityServer:identityServer accessToken:accessToken andNodeRestClient:sdnRestClient];
    ...
    MXWeakify(self);
    sdnRestClient.identityServerAccessTokenHandler = ^MXHTTPOperation *(void (^success)(NSString *accessToken), void (^failure)(NSError *error)) {
...
- (void)_startWithSyncFilterId:(NSString *)syncFilterId onServerSyncDone:(void (^)(void))onServerSyncDone failure:(void (^)(NSError *))failure
{
    ...
    else
    {
        ...
        MXWeakify(self);
        [_myUser updateFromNodeOfSDNSession:self success:^{
            MXStrongifyAndReturnIfNil(self);
    ...
    [self refreshNodeWellknown:nil failure:nil];

    // Refresh node capabilities
    [self refreshNodeCapabilities:nil failure:nil];

    // Refresh supported SDN versions
    [self refreshSupportedSDNVersions:nil failure:nil];
    
    // Get the maximum file size allowed for uploading media
    [self.sdnRestClient maxUploadSize:^(NSInteger maxUploadSize) {
...
- (BOOL)isPauseable
{
    switch (_state)
    {
        ...
        case MXSessionStateNodeNotReachable:
            return YES;
        default:
            return NO;
    }
                   ...
                   failure:(void (^)(NSError *error))failure
{
    ...
    [self enableCrypto:NO success:^{
        ...
        {
            MXHTTPOperation *operation2 = [self.sdnRestClient logout:success failure:failure];
...
- (MXHTTPOperation*)deactivateAccountWithAuthParameters:(NSDictionary*)authParameters
                                           eraseAccount:(BOOL)eraseAccount
                                                ...
                                                failure:(void (^)(NSError *error))failure
{
    return [self.sdnRestClient deactivateAccountWithAuthParameters:authParameters
                        ...
                        setPresence:(NSString*)setPresence
{
    ...
    else
    {
        ...
        MXWeakify(self);
        eventStreamRequest = [sdnRestClient syncFromToken:streamToken serverTimeout:serverTimeout clientTimeout:clientTimeout setPresence:setPresence filter:self.syncFilterId success:^(MXSyncResponse *liveResponse) {
            MXStrongifyAndReturnIfNil(self);
...
- (void)handleServerSyncError:(NSError*)error forRequestWithServerTimeout:(NSUInteger)serverTimeout success:(void (^)(void))success failure:(void (^)(NSError *error))failure
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // Inform the app there is a problem with the connection to the node
                [self setState:MXSessionStateNodeNotReachable];
                ...
                if (networkReachabilityManager.isReachable)
                {
                    ...
                    // Random time it used to avoid all SDN clients to retry all in the same time
...
- (void)handlePresenceEvent:(MXEvent *)event direction:(MXTimelineDirection)direction
{
    ...
    if (userId)
    {
        ...
        [user updateWithPresenceEvent:event inSDNSession:self];
        ...
        for (NSDictionary *event in accountDataUpdate[@"events"])
        {
            ...
            if (event[@"content"] == nil || [event[@"content"] count] == 0) {
                /*
                 ...
                 https://github.com/ShadowJonathan/sdn-doc/blob/account-data-delete/proposals/3391-account-data-delete.md#sync
                 */
...
- (void)enableVoIPWithCallStack:(id<MXCallStack>)callStack
{
    ...
    _callManager = [[MXCallManager alloc] initWithSDNSession:self andCallStack:callStack];
}
...
- (void)setAntivirusServerURL:(NSString *)antivirusServerURL
{
    ...
    [sdnRestClient setAntivirusServer:antivirusServerURL];
    
    // Configure scan manager if antivirusServerURL is set
    if (antivirusServerURL)
    {
        _scanManager = [[MXScanManager alloc] initWithRestClient:sdnRestClient];
...
- (void)onCreatedRoom:(MXCreateRoomResponse*)response success:(void (^)(MXRoom *room))success
{
    ...
    if (success)
    {
        ...
        if (room)
        {
            // Initialise notification counters node side
            [room markAllAsRead];

            // The first /sync response for this room may have happened before the
            // node answer to the createRoom request.
            success(room);
...
- (MXHTTPOperation*)createRoom:(NSString*)name
                    visibility:(MXRoomDirectoryVisibility)visibility
                     roomAlias:(NSString*)roomAlias
                       ...
                       failure:(void (^)(NSError *error))failure
{
    return [sdnRestClient createRoom:name visibility:visibility roomAlias:roomAlias topic:topic success:^(MXCreateRoomResponse *response) {
...
- (MXHTTPOperation*)createRoomWithParameters:(MXRoomCreationParameters*)parameters
                                     ...
                                     failure:(void (^)(NSError *error))failure
{
    return [sdnRestClient createRoomWithParameters:parameters success:^(MXCreateRoomResponse *response) {
...
- (MXHTTPOperation*)createRoom:(NSDictionary*)parameters
                       ...
                       failure:(void (^)(NSError *error))failure
{
    return [sdnRestClient createRoom:parameters success:^(MXCreateRoomResponse *response) {
...
- (void)onJoinedRoom:(NSString*)roomId success:(void (^)(MXRoom *room))success
{
    ...
    if (success)
    {
        if (room.summary.membership == MXMembershipJoin)
        {
            ...
            // node answer to the joinRoom request.
            success(room);
...
- (MXHTTPOperation*)joinRoom:(NSString*)roomIdOrAlias
    ...
    return [sdnRestClient joinRoom:roomIdOrAlias viaServers:viaServers withThirdPartySigned:nil success:^(NSString *theRoomId) {
                      ...
                      failure:(void (^)(NSError *error))failure
{
    ...
    return [sdnRestClient leaveRoom:roomId success:^{

        // Check the room has been removed before calling the success callback
        // This is automatically done when the node sends the MXMembershipLeave event.
                                               ...
                                               failure:(void (^)(NSError *))failure
{
    ...
    // Push the current direct rooms dictionary to the node
...
{
    MXRoom *room = [[MXRoom alloc] initWithRoomId:roomId andSDNSession:self];
...
{
    MXRoomSummary *summary = [[MXRoomSummary alloc] initWithRoomId:roomId andSDNSession:self];
...
{
    MXRoom *room = [MXRoom loadRoomFromStore:self.store withRoomId:roomId sdnSession:self];
                             ...
                             failure:(void (^)(NSError *error))failure
{
    ...
    if (roomId)
    {
        ...
        {
            operation = [sdnRestClient eventWithEventId:eventId inRoom:roomId success:decryptIfNeeded failure:failure];
        }
    }
    else
    {
        operation = [sdnRestClient eventWithEventId:eventId success:decryptIfNeeded failure:failure];
    }
    ...
    if (roomId)
    {
        ...
        if (roomSummary == nil)
        {
            ...
            if (roomSummaryProtocol)
            {
                ...
                [roomSummary setSDNSession:self];
                              ...
                              failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient acceptGroupInvite:groupId success:^{
        MXStrongifyAndReturnIfNil(self);
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient leaveGroup:groupId success:^{
        MXStrongifyAndReturnIfNil(self);
                                 ...
                                 failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient updateGroupPublicity:group.groupId isPublicised:isPublicised success:^(void) {
        MXStrongifyAndReturnIfNil(self);
        ...
        [self->userIdsWithOutdatedPublicisedGroups addObject:self->sdnRestClient.credentials.userId];
        [self publicisedGroupsForUser:self->sdnRestClient.credentials.userId];
                            ...
                            failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient getGroupProfile:group.groupId success:^(MXGroupProfile *groupProfile) {
        MXStrongifyAndReturnIfNil(self);
                            ...
                            failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient getGroupSummary:group.groupId success:^(MXGroupSummary *groupSummary) {
        MXStrongifyAndReturnIfNil(self);
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient getGroupUsers:group.groupId success:^(MXGroupUsers *groupUsers) {
        MXStrongifyAndReturnIfNil(self);
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient getGroupInvitedUsers:group.groupId success:^(MXGroupUsers *invitedUsers) {
        MXStrongifyAndReturnIfNil(self);
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sdnRestClient getGroupRooms:group.groupId success:^(MXGroupRooms *groupRooms) {
        MXStrongifyAndReturnIfNil(self);
...
- (void)peekInRoomWithRoomId:(NSString*)roomId
                     ...
                     failure:(void (^)(NSError *error))failure
{
    MXPeekingRoom *peekingRoom = [[MXPeekingRoom alloc] initWithRoomId:roomId andSDNSession:self];
...
{
    // Algo (and the [0.0, 1.0] assumption) inspired from sdn-react-sdk:
...
{
    MXWeakify(self);
    return [sdnRestClient setAccountData:data forType:type success:^{
        MXStrongifyAndReturnIfNil(self);
...
{
    MXWeakify(self);
    return [sdnRestClient deleteAccountDataWithType:type success:^{
        MXStrongifyAndReturnIfNil(self);
                                          ...
                                          failure:(void (^)(NSError *))failure
{
    ...
    if (identityServer)
    {
        ...
        __block MXIdentityService *identityService = [[MXIdentityService alloc] initWithIdentityServer:identityServer accessToken:nil andNodeRestClient:sdnRestClient];
                                          ...
                                          failure:(void (^)(NSError *error))failure
{
    ...
    if (!identityService)
    {
        ...
        identityService = [[MXIdentityService alloc] initWithIdentityServer:baseURL
                                                                accessToken:nil
                                                    andNodeRestClient:self.sdnRestClient];
    }
...
- (MXWellKnown *)nodeWellknown
{
    return self.store.nodeWellknown;
}

- (MXHTTPOperation *)refreshNodeWellknown:(void (^)(MXWellKnown *))success
                                        failure:(void (^)(NSError *))failure
{
    MXLogDebug(@"[MXSession] refreshNodeWellknown");
    if (!autoDiscovery)
    {
        NSString *node = [MXSDKOptions sharedInstance].wellknownDomainUrl;
        if (!node)
        {
            // Retrieve the domain from the user id as it can be different from the `MXRestClient.node` that uses the client-server API endpoint domain.
            NSString *userDomain = [MXTools serverNameInSDNIdentifier:self.myUserId];
            
            if (userDomain)
            {
                node =  [NSString stringWithFormat:@"https://%@", userDomain];
            }
            else
            {
                node = sdnRestClient.node;
            }
        }
    ...
    return [autoDiscovery wellKnow:^(MXWellKnown * _Nonnull wellKnown) {
        ...
        [self.store storeNodeWellknown:wellKnown];
...
#pragma mark - Node capabilities

- (MXCapabilities *)nodeCapabilities
{
    return self.store.nodeCapabilities;
}

- (MXHTTPOperation *)refreshNodeCapabilities:(void (^)(MXCapabilities *))success
                                           failure:(void (^)(NSError *))failure
{
    MXLogDebug(@"[MXSession] refreshNodeCapabilities");

    MXWeakify(self);
    return [self.sdnRestClient capabilities:^(MXCapabilities *capabilities) {
        MXStrongifyAndReturnIfNil(self);

        if (capabilities)
        {
            [self.store storeNodeCapabilities:capabilities];
        }
...
#pragma mark - Supported SDN versions

- (MXHTTPOperation*)supportedSDNVersions:(void (^)(MXSDNVersions *))success failure:(void (^)(NSError *))failure
{
    MXSDNVersions *supportedVersionsInStore = self.store.supportedSDNVersions;
    ...
    return [sdnRestClient supportedSDNVersions:success failure:failure];
}

- (MXHTTPOperation *)refreshSupportedSDNVersions:(void (^)(MXSDNVersions *))success
                                            failure:(void (^)(NSError *))failure
{
    MXLogDebug(@"[MXSession] refreshSDNSupportedVersions");

    MXWeakify(self);
    return [self.sdnRestClient supportedSDNVersions:^(MXSDNVersions *sdnVersions) {
        MXStrongifyAndReturnIfNil(self);

        if (sdnVersions)
        {
            [self.store storeSupportedSDNVersions:sdnVersions];
        }
                      ...
                      failure:(void (^)(NSError *error))failure
{
    ...
    if (self.store)
    {
        ...
        [self.store filterIdForFilter:filter success:^(NSString * _Nullable filterId) {
            ...
            {
                // Else, create node side
                MXWeakify(self);
                MXHTTPOperation *operation2 = [self.sdnRestClient setFilter:filter success:^(NSString *filterId) {
                    MXStrongifyAndReturnIfNil(self);
    ...
    {
        operation = [self.sdnRestClient setFilter:filter success:success failure:failure];
    }
                               ...
                               failure:(void (^)(NSError *error))failure
{
    ...
    if (self.store)
    {
        ...
        [self.store filterWithFilterId:filterId success:^(MXFilterJSONModel * _Nullable filter) {
            ...
            else
            {
                // Check on the node
                MXWeakify(self);
                MXHTTPOperation *operation2 = [self.sdnRestClient getFilterWithFilterId:filterId success:^(MXFilterJSONModel *filter) {
                    MXStrongifyAndReturnIfNil(self);
    ...
    {
         operation = [sdnRestClient getFilterWithFilterId:filterId success:success failure:failure];
    }
...
- (NSArray<NSString *> *)publicisedGroupsForUser:(NSString*)userId
{
    ...
    if (userId)
    {
        ...
        {
            MXWeakify(self);
            [self.sdnRestClient getPublicisedGroupsForUsers:@[userId] success:^(NSDictionary<NSString *,NSArray<NSString *> *> *updatedPublicisedGroupsByUserId) {
                MXStrongifyAndReturnIfNil(self);
