...
extension MXThreadingServiceError: CustomNSError {
    public static let errorDomain = "org.sdn.sdk.threadingservice"
...
public class MXThreadingService: NSObject {
    ...
    public func allThreads(inRoom roomId: String,
        ...
        dispatchGroup.enter()
        let operation = session.supportedSDNVersions { response in
            switch response {
        ...
        dispatchGroup.notify(queue: .main) {
            if serverSupportThreads {
                let newOperation = session.sdnRestClient.threadsInRoomWithId(roomId, include: onlyParticipated ? .participated : .all, from: from) { response in
                    switch response {
    ...
    private func thread(forRootEvent rootEvent: MXEvent, session: MXSession) -> MXThreadModel {
        ...
        //  workaround for https://github.com/sdn-org/synapse/issues/11753. Can be removed when that's fixed.
