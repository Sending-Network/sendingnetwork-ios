...
{
    /**
     The SDN session to make be able to make Home Server requests.
     */
    MXSession *mxSession;
...
- (instancetype)initWithSDNSession:(MXSession *)mxSession2
{
    self = [super init];
    if (self)
    {
        ...
        // Define condition checkers for default SDN conditions
        ...
        MXPushRuleDisplayNameCondtionChecker *displayNameCondtionChecker = [[MXPushRuleDisplayNameCondtionChecker alloc] initWithSDNSession:mxSession currentUserDisplayName:nil];
        [self setChecker:displayNameCondtionChecker forConditionKind:kMXPushRuleConditionStringContainsDisplayName];

        MXPushRuleRoomMemberCountConditionChecker *roomMemberCountConditionChecker = [[MXPushRuleRoomMemberCountConditionChecker alloc] initWithSDNSession:mxSession];
        [self setChecker:roomMemberCountConditionChecker forConditionKind:kMXPushRuleConditionStringRoomMemberCount];

        MXPushRuleSenderNotificationPermissionConditionChecker *notificationPermissionConditionChecker = [[MXPushRuleSenderNotificationPermissionConditionChecker alloc] initWithSDNSession:mxSession];
        ...
        [mxSession listenToEvents:^(MXEvent *event, MXTimelineDirection direction, id customObject) {
            ...
            if (MXTimelineDirectionForwards == direction
                && NO == [event.sender isEqualToString:self->mxSession.sdnRestClient.credentials.userId])
            {
...
{
    return [mxSession.sdnRestClient pushRules:^(MXPushRulesResponse *pushRules) {
...
- (MXPushRule *)ruleMatchingEvent:(MXEvent *)event roomState:(MXRoomState*)roomState
{
    ...
    if (NO == [event.sender isEqualToString:mxSession.sdnRestClient.credentials.userId])
    {
...
- (void)removeRule:(MXPushRule*)pushRule
{
    if (pushRule)
    {
        ...
        MXWeakify(self);
        [mxSession.sdnRestClient removePushRule:pushRule.ruleId scope:pushRule.scope kind:pushRule.kind success:^{
            MXStrongifyAndReturnIfNil(self);
...
- (void)enableRule:(MXPushRule*)pushRule isEnabled:(BOOL)enable completion:(nullable void (^)(NSError * _Nullable error))completion;
{
    if (pushRule)
    {
        ...
        MXWeakify(self);
        [mxSession.sdnRestClient enablePushRule:pushRule.ruleId scope:pushRule.scope kind:pushRule.kind enable:enable success:^{
            MXStrongifyAndReturnIfNil(self);
                   ...
                   completion:(nullable void (^)(NSError * _Nullable error))completion
{
    ...
    [mxSession.sdnRestClient updateActionsForPushRule:ruleId
            ...
            highlight:(BOOL)highlight
{
    ...
    [mxSession.sdnRestClient addPushRule:ruleId scope:kMXPushRuleScopeStringGlobal kind:kind actions:actions pattern:pattern conditions:conditions success:^{
