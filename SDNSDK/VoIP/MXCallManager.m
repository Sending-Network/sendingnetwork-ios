...
NSString *const kMXProtocolVectorPSTN = @"im.vectSending.protocol.pstn";
NSString *const kMXProtocolPSTN = @"m.protocol.pstn";

NSString *const kMXProtocolVectorSipNative = @"im.vectSending.protocol.sip_native";
NSString *const kMXProtocolVectorSipVirtual = @"im.vectSending.protocol.sip_virtual";
...
@interface MXCallManager ()
{
    ...
    /**
     Listener to SDN call-related events.
     */
...
- (instancetype)initWithSDNSession:(MXSession *)mxSession andCallStack:(id<MXCallStack>)callstack
{
    self = [super init];
    if (self)
    {
...
{
    MXWeakify(self);
    [_mxSession.sdnRestClient turnServer:^(MXTurnServerResponse *turnServerResponse) {
        MXStrongifyAndReturnIfNil(self);
...
// FIXME: This currently forces Vector to try to hit the sdn.org AS for conferencing.
// This is bad because it prevents people running their own ASes from being used.
// This isn't permanent and will be customisable in the future: see the proposal
// at docs/conferencing.md for more info.
NSString *const kMXCallManagerConferenceUserPrefix  = @"@fs_";
NSString *const kMXCallManagerConferenceUserDomain  = @"sdn.org";
...
{
    // Apply the same algo as other sdn clients
    ...
    if ([userId hasPrefix:kMXCallManagerConferenceUserPrefix])
    {
        ...
        if (base64part)
        {
            ...
            if (decodedData)
            {
                ...
                if (decoded)
                {
                    isConferenceUser = [MXTools isSDNRoomIdentifier:decoded];
                }
            }
        }
    }
...
{
    MXWeakify(self);
    [_mxSession.sdnRestClient thirdpartyProtocols:^(MXThirdpartyProtocolsResponse *response) {
        MXStrongifyAndReturnIfNil(self);
...
{
    [_mxSession.sdnRestClient thirdpartyUsers:kMXProtocolVectorPSTN
                                          fields:@{
...
- (void)getVirtualUserFrom:(NSString *)userId
                   ...
                   failure:(void (^)(NSError * _Nullable))failure
{
    [_mxSession.sdnRestClient thirdpartyUsers:kMXProtocolVectorSipVirtual
                                          fields:@{
                                              @"native_mxid": userId
                                          }
                                         success:^(MXThirdPartyUsersResponse *thirdpartyUsersResponse) {
...
- (void)getNativeUserFrom:(NSString *)userId
                  ...
                  failure:(void (^)(NSError * _Nullable))failure
{
    [_mxSession.sdnRestClient thirdpartyUsers:kMXProtocolVectorSipNative
                                          fields:@{
                                              @"virtual_mxid": userId
                                          }
                                         success:^(MXThirdPartyUsersResponse *thirdpartyUsersResponse) {
