...
- (instancetype)initWithRoomId:(NSString *)roomId andSDNSession:(MXSession *)mxSession
{
    ...
    return [self initWithRoomId:roomId sdnSession:mxSession andStore:mxSession.store];
}

- (instancetype)initWithRoomId:(NSString *)roomId sdnSession:(MXSession *)mxSession andStore:(id<MXStore>)theStore
{
    self = [self init];
    if (self)
    {
...
- (void)setSDNSession:(MXSession *)mxSession
{
    if (!_mxSession)
    {
...
- (void)resetRoomStateData
{
    ...
    [room state:^(MXRoomState *roomState) {
        ...
        if (self.displayName == nil || self.avatar == nil)
        {
            ...
            // we reapply the SDN name/avatar calculation algorithm.
                                                        ...
                                                        operation:(MXHTTPOperation *)operation commit:(BOOL)commit
{
    ...
    {
        // If requested, get messages from the node
...
- (void)eventDidChangeSentState:(NSNotification *)notif
{
    ...
    // updated by the true event coming back from the node.
