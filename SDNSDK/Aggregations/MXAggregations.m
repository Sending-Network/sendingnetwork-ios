...
- (MXHTTPOperation*)reactionsEventsForEvent:(NSString*)eventId
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    return [self.mxSession.sdnRestClient relationsForEvent:eventId
                                                       inRoom:roomId
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    ...
    if (!event)
    {
        operation = [self.mxSession.sdnRestClient relationsForEvent:eventId
                                                                inRoom:roomId
...
- (instancetype)initWithSDNSession:(MXSession *)mxSession
{
    ...
    if (self)
    {
        ...
        self.store = [[MXRealmAggregationsStore alloc] initWithCredentials:mxSession.sdnRestClient.credentials];

        self.aggregatedReactionsUpdater = [[MXAggregatedReactionsUpdater alloc] initWithSDNSession:self.mxSession aggregationStore:self.store];
        self.aggregatedEditsUpdater = [[MXAggregatedEditsUpdater alloc] initWithSDNSession:self.mxSession
                                                                             aggregationStore:self.store
                                                                                  sdnStore:mxSession.store];
        self.aggregatedReferencesUpdater = [[MXAggregatedReferencesUpdater alloc] initWithSDNSession:self.mxSession
                                                                                           sdnStore:mxSession.store];
