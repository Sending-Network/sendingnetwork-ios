...
@interface MXFileStore ()
{
    ...
    // Flag to indicate SDN filters need to be stored
    ...
    dispatch_async(dispatchQueue, ^(void){
        ...
        @autoreleasepool
        {
            ...
            if (self->metaData && kMXFileVersion != self->metaData.version)
            {
                ...
                {
                    MXLogDebug(@"[MXFileStore] SDN SDK until the version of 35 of MXFileStore caches all NSURLRequests unnecessarily. Clear NSURLCache");
            ...
            if (nil == self->metaData && self->credentials.node && self->credentials.userId)
            {
                self->metaData = [[MXFileStoreMetaData alloc] init];
                self->metaData.node = [self->credentials.node copy];
...
{
    [super storeHasReachedNodePaginationEndForRoom:roomId andValue:value];
...
{
    // XXX: To remove once https://github.com/Sending-Network/sendingnetwork-ios/issues/3807 is fixed
...
{
    return metaData.nodeWellknown;
}

- (void)storeNodeWellknown:(nonnull MXWellKnown *)wellknown
{
    if (metaData)
    {
        metaData.nodeWellknown = wellknown;
...
{
    return metaData.nodeCapabilities;
}

- (void)storeNodeCapabilities:(MXCapabilities *)capabilities
{
    if (metaData)
    {
        metaData.nodeCapabilities = capabilities;
        metaDataHasChanged = YES;
    }
}
...
{
    return metaData.supportedSDNVersions;
}
...
- (void)storeSupportedSDNVersions:(MXSDNVersions *)supportedSDNVersions
{
    if (metaData)
    {
        metaData.supportedSDNVersions = supportedSDNVersions;
        metaDataHasChanged = YES;
    }
}
