...
//  Copyright © 2017 sdn.org. All rights reserved.
...
public extension MXSession {
    ...
    @nonobjc @discardableResult func createRoom(withName name: String,
        ...
        case .restricted:
            ...
            let joinRuleSupportType = nodeCapabilitiesService.isFeatureSupported(.restricted)
            if joinRuleSupportType == .supported || joinRuleSupportType == .supportedUnstable {
                ...
                parameters.roomVersion = nodeCapabilitiesService.versionOverrideForFeature(.restricted)
    ...
    */
    ...
    /// Fetch an event from the session store, or from the node if required.
    /// - Parameters:
    ...
    var nodeCapabilities: MXCapabilities? {
        return __nodeCapabilities
    }

    /// SDN versions supported by the node.
    /// 
    /// - Parameter completion: A block object called when the operation completes.
    /// - Returns: a `MXHTTPOperation` instance.
    @nonobjc @discardableResult func supportedSDNVersions(_ completion: @escaping (_ response: MXResponse<MXSDNVersions>) -> Void) -> MXHTTPOperation {
