 ...
 */
NSString *const MXServiceTermsErrorDomain = @"org.sdn.sdk.MXServiceTermsErrorDomain";
...
- (instancetype)initWithBaseUrl:(NSString*)baseUrl serviceType:(MXServiceType)serviceType sdnSession:(nullable MXSession *)mxSession accessToken:(nullable NSString *)accessToken
{
    self = [super init];
    if (self)
    {
                          ...
                          failure:(void (^)(NSError * _Nonnull))failure
{
    if (!_mxSession || !_accessToken)
    {
        if (failure)
        {
            NSError *error = [NSError errorWithDomain:MXServiceTermsErrorDomain code:MXServiceTermsErrorMissingParameters userInfo:@{ NSLocalizedDescriptionKey : @"No SDN session or no access token"}];
            failure(error);
        }
        return nil;
    }
