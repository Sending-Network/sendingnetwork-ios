...
@interface MXLegacyCrypto () <MXRecoveryServiceDelegate, MXUnrequestedForwardedRoomKeyManagerDelegate>
{
    ...
    // Last time we check available one-time keys on the node
...
+ (id<MXCrypto>)createCryptoWithSDNSession:(MXSession *)mxSession
                                        error:(NSError **)error
{
    __block id<MXCrypto> crypto;

#ifdef MX_CRYPTO
    dispatch_queue_t cryptoQueue = [MXLegacyCrypto dispatchQueueForUser:mxSession.sdnRestClient.credentials.userId];
    dispatch_sync(cryptoQueue, ^{

        MXCryptoStoreClass *cryptoStore = [MXCryptoStoreClass createStoreWithCredentials:mxSession.sdnRestClient.credentials];
        cryptoStore.cryptoVersion = MXCryptoVersionLast;
        crypto = [[MXLegacyCrypto alloc] initWithSDNSession:mxSession cryptoQueue:cryptoQueue andStore:cryptoStore];
...
+ (void)initializeCryptoWithSDNSession:(MXSession *)mxSession
                        migrationProgress:(void (^)(double))migrationProgress
                                 complete:(void (^)(id<MXCrypto> crypto, NSError *error))complete
{
#ifdef MX_CRYPTO
    [self initalizeLegacyCryptoWithSDNSession:mxSession complete:complete];
#else
    complete(nil);
#endif
}

+ (void)initalizeLegacyCryptoWithSDNSession:(MXSession*)mxSession complete:(void (^)(id<MXCrypto> crypto, NSError *error))complete
{
#ifdef MX_CRYPTO

    MXLogDebug(@"[MXCrypto] checkCryptoWithSDNSession for %@", mxSession.sdnRestClient.credentials.userId);
    ...
    dispatch_async(cryptoQueue, ^{
        ...
        if ([MXCryptoStoreClass hasDataForCredentials:mxSession.sdnRestClient.credentials])
        {
            MXLogDebug(@"[MXCrypto] checkCryptoWithSDNSession: Crypto store exists");

            // If it already exists, init store and crypto
            MXCryptoStoreClass *cryptoStore = [[MXCryptoStoreClass alloc] initWithCredentials:mxSession.sdnRestClient.credentials];

            MXLogDebug(@"[MXCrypto] checkCryptoWithSDNSession: Crypto store initialized");

            id<MXCrypto> crypto = [[MXLegacyCrypto alloc] initWithSDNSession:mxSession cryptoQueue:cryptoQueue andStore:cryptoStore];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                complete(crypto, nil);
                 ...
                 && mxSession.sdnRestClient.credentials.deviceId)
        {
            MXLogDebug(@"[MXCrypto] checkCryptoWithSDNSession: Need to create the store");
...
- (void)deleteStore:(void (^)(void))onComplete;
{
    ...
    dispatch_async(_cryptoQueue, ^{
        ...
        [MXCryptoStoreClass deleteStoreWithCredentials:self.mxSession.sdnRestClient.credentials];
      ...
      failure:(void (^)(NSError *error))failure
{
    ...
    startOperation = [self uploadDeviceKeys:^(MXKeysUploadResponse *keysUploadResponse) {
        ...
        // TODO: sdn-js-sdk does not do it anymore and waits for the completion
...
- (void)close:(BOOL)deleteStore
{
    ...
    dispatch_sync(_cryptoQueue, ^{
        ...
        [self.sdnRestClient close];
        self->_sdnRestClient = nil;
...
- (void)handleDeviceOneTimeKeysCount:(NSDictionary<NSString *, NSNumber*>*)deviceOneTimeKeysCount
{
    ...
    MXLogDebug(@"[MXCrypto] handleDeviceOneTimeKeysCount: %@ keys on the node", deviceOneTimeKeysCount[kMXKeySignedCurve25519Type]);
...
- (void)onSyncCompleted:(NSString *)oldSyncToken nextSyncToken:(NSString *)nextSyncToken catchingUp:(BOOL)catchingUp
{
    ...
    dispatch_async(_cryptoQueue, ^{
        ...
        // (https://github.com/Sending-Network/SendingnNetwork-web/issues/2782).
        if (!catchingUp)
        {
...
- (void)setUserVerification:(BOOL)verificationStatus forUser:(NSString*)userId
                    ...
                    failure:(void (^)(NSError *error))failure
{
    // We cannot remove cross-signing trust for a user in the sdn spec
...
- (instancetype)initWithSDNSession:(MXSession*)sdnSession cryptoQueue:(dispatch_queue_t)theCryptoQueue andStore:(id<MXCryptoStore>)store
{
    ...
    if (self)
    {
        ...
        decryptionQueue = [MXLegacyCrypto dispatchQueueForUser:_mxSession.sdnRestClient.credentials.userId];
        ...
        _sdnRestClient = [[MXRestClient alloc] initWithCredentials:_mxSession.sdnRestClient.credentials andOnUnrecognizedCertificateBlock:nil andPersistentTokenDataHandler:_mxSession.sdnRestClient.persistTokenDataHandler andUnauthenticatedHandler:_mxSession.sdnRestClient.unauthenticatedHandler];
        _sdnRestClient.completionQueue = _cryptoQueue;
        ...
        if (!deviceId)
        {
            // Generate a device id if the node did not provide it or it was lost
        ...
        NSString *userId = _sdnRestClient.credentials.userId;
        ...
        outgoingRoomKeyRequestManager = [[MXOutgoingRoomKeyRequestManager alloc]
                                         initWithSDNRestClient:_sdnRestClient
        ...
        _secretStorage = [[MXSecretStorage alloc] initWithSDNSession:_mxSession processingQueue:_cryptoQueue];
        ...
        if ([MXSDKOptions sharedInstance].enableKeyBackupWhenStartingMXCrypto)
        {
            ...
            _backup = [[MXKeyBackup alloc] initWithEngine:engine
                                               restClient:_sdnRestClient
                                       secretShareManager:_secretShareManager
                                                    queue:_cryptoQueue];
        }
        
        MXRecoveryServiceDependencies *dependencies = [[MXRecoveryServiceDependencies alloc] initWithCredentials:_mxSession.sdnRestClient.credentials
                                                                                                          backup:_backup
...
- (MXDeviceInfo *)eventSenderDeviceOfEvent:(MXEvent *)event
{
    ...
    //
    // (see https://github.com/Sending-Network/vector-web/issues/2215)
...

{
    ...
    return [_sdnRestClient claimOneTimeKeysForUsersDevices:usersDevicesToClaim success:^(MXKeysClaimResponse *keysClaimResponse) {
...
- (NSDictionary*)encryptMessage:(NSDictionary*)payloadFields forDevices:(NSArray<MXDeviceInfo*>*)devices
{
    ...
    for (MXDeviceInfo *recipientDevice in devices)
    {
        ...
        if (sessionId)
        {
            ...
            payloadJson[@"sender"] = _sdnRestClient.credentials.userId;
            ...
            // https://github.com/Sending-Network/vector-web/issues/2483
...
- (void)invalidateDeviceListsSince:(NSString*)oldSyncToken to:(NSString*)lastKnownSyncToken
                           ...
                           failure:(void (^)(NSError *error))failure
{
    [_sdnRestClient keyChangesFrom:oldSyncToken to:lastKnownSyncToken success:^(MXDeviceListResponse *deviceLists) {
...
{
    // Sanity check
    if (!_sdnRestClient.credentials.userId)
    {
        MXLogError(@"[MXCrypto] uploadDeviceKeys. ERROR: _sdnRestClient.credentials.userId cannot be nil");
        failure(nil);
        return nil;
    }
    ...
    _myDevice.signatures = @{
                            _sdnRestClient.credentials.userId: @{
    ...
    return [_sdnRestClient uploadKeys:_myDevice.JSONDictionary oneTimeKeys:nil fallbackKeys:nil success:success failure:failure];
}
...
- (void)maybeUploadOneTimeKeys:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    ...
    if (oneTimeKeyCount != -1)
    {
        ...
        MXLogDebug(@"[MXCrypto] maybeUploadOneTimeKeys: there are %tu one-time keys on the node", oneTimeKeyCount);
...
{
    MXLogDebug(@"[MXCrypto] generateAndUploadOneTimeKeys: %@ one time keys are available on the node", @(keyCount));
    ...
    if ([self generateOneTimeKeys:keyCount])
    {
        operation = [self uploadOneTimeKeys:^(MXKeysUploadResponse *keysUploadResponse) {
            ...
            {
                // The node explicitly rejected the request.
                // Reset local OTKs we tried to push and retry
                // There is no sdn specific error but we really want to detect the error described at
                // https://github.com/Sending-Network/sendingnetwork-ios/issues/3721
...
- (MXHTTPOperation *)generateAndUploadFallbackKey
{
    ...
    MXWeakify(self);
    return [_sdnRestClient uploadKeys:nil oneTimeKeys:nil fallbackKeys:fallbackKeyJson success:^(MXKeysUploadResponse *keysUploadResponse) {
        MXStrongifyAndReturnIfNil(self);
...
/**
 ...
 @param keyCount the number of key currently available on the node.
...
- (BOOL)generateOneTimeKeys:(NSUInteger)keyCount
{
    ...
    MXLogDebug(@"[MXCrypto] generateOneTimeKeys: %tu one-time keys on the node", keyCount);
...
- (MXHTTPOperation *)uploadOneTimeKeys:(void (^)(MXKeysUploadResponse *keysUploadResponse))success failure:(void (^)(NSError *))failure
{
    ...
    // same one as used in login.
    MXWeakify(self);
    return [_sdnRestClient uploadKeys:nil oneTimeKeys:oneTimeJson fallbackKeys:nil success:^(MXKeysUploadResponse *keysUploadResponse) {
        MXStrongifyAndReturnIfNil(self);
...
{
    return [_sdnRestClient uploadKeys:_myDevice.JSONDictionary oneTimeKeys:nil fallbackKeys:nil success:^(MXKeysUploadResponse *keysUploadResponse) {
        
        NSUInteger publishedkeyCount = [keysUploadResponse oneTimeKeyCountsForAlgorithm:kMXKeySignedCurve25519Type];
        
        MXLogDebug(@"[MXCrypto] publishedOneTimeKeysCount: %@ OTKs on the node", @(publishedkeyCount));
...
- (void)markOlmSessionForUnwedgingInEvent:(MXEvent*)event
{
    ...
    [self ensureOlmSessionsForDevices:userDevice force:YES success:^(MXUsersDevicesMap<MXOlmSessionResult *> *results) {
        ...
        [self.sdnRestClient sendToDevice:payload success:nil failure:^(NSError *error) {
