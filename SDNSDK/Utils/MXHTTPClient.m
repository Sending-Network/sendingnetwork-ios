...
NSString* const MXHTTPClientErrorResponseDataKey = @"com.sdnsdk.httpclient.error.response.data";
...
NSString* const kMXHTTPClientSDNErrorNotification = @"kMXHTTPClientSDNErrorNotification";
NSString* const kMXHTTPClientSDNErrorNotificationErrorKey = @"kMXHTTPClientSDNErrorNotificationErrorKey";
...
//    NSString *URLString = [[NSURL URLWithString:path relativeToURL:httpManager.baseURL] absoluteString];
    ...
    mxHTTPOperation.operation = [httpManager dataTaskWithRequest:request uploadProgress:^(NSProgress * _Nonnull theUploadProgress) {
        ...
        else
        {
            ...
            if (response)
            {
                // If the home server (or any other SDN server) sent data, it may contain 'errcode' and 'error'.
                ...
                if (JSONResponse)
                {
                    ...
                    if (JSONResponse[kMXErrorCodeKey] || JSONResponse[kMXErrorMessageKey])
                    {
                        ...
                        // Send a notification
                        [[NSNotificationCenter defaultCenter] postNotificationName:kMXHTTPClientSDNErrorNotification
                                                                            object:self
                                                                          userInfo:@{ kMXHTTPClientSDNErrorNotificationErrorKey: mxError }];
