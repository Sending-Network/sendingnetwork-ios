...
- (id)initForUploadWithSDNSession:(MXSession*)sdnSession initialRange:(CGFloat)initialRange andRange:(CGFloat)range
{
    if (self = [super init])
    {
...
- (void)uploadData:(NSData *)data filename:(NSString*)filename mimeType:(NSString *)mimeType success:(blockMXMediaLoader_onSuccess)success failure:(blockMXMediaLoader_onError)failure
{
    ...
    MXWeakify(self);
    operation = [mxSession.sdnRestClient uploadContent:data
                                                 filename:filename
                                                 mimeType:mimeType
                                                  timeout:30
