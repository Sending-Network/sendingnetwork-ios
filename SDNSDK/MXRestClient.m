...
NSString *const kMXAccountDataTypeBreadcrumbs = @"im.vectSending.setting.breadcrumbs";
NSString *const kMXAccountDataTypeClientInformation = @"io.sendingnetwork.sdn_client_information";
...
NSString *const kMXAccountDataLocalNotificationKeyPrefix = @"org.sdn.msc3890.local_notification_settings.";
...
-(id)initWithNode:(NSString *)node andOnUnrecognizedCertificateBlock:(MXHTTPClientOnUnrecognizedCertificate)onUnrecognizedCertBlock
{
...
andUnauthenticatedHandler: (MXRestClientUnauthenticatedHandler)unauthenticatedHandler
{
    ...
    if (self)
    {
        ...
        if (credentials.node)
        {
            httpClient = [[MXHTTPClient alloc] initWithBaseURL:credentials.node
...
+ (MXCredentials*)findMatchingCredential:(MXCredentials*)credential inCredentials:(NSArray<MXCredentials *> *)credentials
{
    ...
    {
        if (nextCredential.node && [nextCredential.node isEqualToString:credential.node]
...
- (NSString *)nodeSuffix
{
    ...
    // Extract node suffix from userId
    NSArray *components = [credentials.userId componentsSeparatedByString:@":"];
    if (components.count > 1)
    {
        // Remove first component
        NSString *sdnId = components.firstObject;
        NSRange range = NSMakeRange(0, sdnId.length);
        nodeSuffix = [credentials.userId stringByReplacingCharactersInRange:range withString:@""];
    }
    else
    {
    ...
    return [httpClient requestWithMethod:@"GET"
                              ...
                              parameters:nil
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXSDNVersions *sdnVersions;
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(sdnVersions, MXSDNVersions, JSONResponse);
                                         } andCompletion:^{
                                             success(sdnVersions);
                                         }];
                                     }
...
- (MXHTTPOperation*)wellKnow:(void (^)(MXWellKnown *wellKnown))success
                     failure:(void (^)(NSError *error))failure
{
    NSString *path = @".well-known/sdn/client";
...
- (NSString*)registerFallback;
{
    ...
    if (self.credentials.node)
    {
        registerFallback = [[NSURL URLWithString:@"_api/static/client/register/" relativeToURL:[NSURL URLWithString:self.credentials.node]] absoluteString];
    }
...
- (NSString*)loginFallback;
{
    ...
    if (self.credentials.node)
    {
        loginFallback = [[NSURL URLWithString:@"/_api/static/client/login/" relativeToURL:[NSURL URLWithString:self.credentials.node]] absoluteString];
    }
                                          ...
                                          failure:(void (^)(NSError *error))failure
{
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:[NSString stringWithFormat:@"%@/org.sdn.msc3882/login/token", kMXAPIPrefixPathUnstable]
                              parameters:@{}
                                 success:^(NSDictionary *JSONResponse) {
        MXStrongifyAndReturnIfNil(self);
...
- (MXHTTPOperation*)registerOrLogin:(MXAuthAction)authAction parameters:(NSDictionary *)parameters success:(void (^)(NSDictionary *JSONResponse))success failure:(void (^)(NSError *))failure
{
    ...
    // Do it only if parameters contains the password field, do make node happy.
...
- (MXHTTPOperation*)deleteAccountDataWithType:(NSString*)type
                                      ...
                                      failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/org.sdn.msc3391/user/%@/account_data/%@", kMXAPIPrefixPathUnstable, credentials.userId, type];
                                     ...
                                     failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    operation = [self supportedSDNVersions:^(MXSDNVersions *sdnVersions) {
        MXStrongifyAndReturnIfNil(self);

        if (!sdnVersions.doesServerSupportSeparateAddAndBind && self.identityServer)
        {
                                              ...
                                              failure:(void (^)(NSError *error))failure
{
    ...
    operation = [self supportedSDNVersions:^(MXSDNVersions *sdnVersions) {
        ...
        if (sdnVersions.doesServerAcceptIdentityAccessToken)
        {
                        ...
                        failure:(void (^)(NSError *error))failure
{
    ...
    {
        NSString* property = withRelationsIsStable ? @"with_rel_types" : @"org.sdn.msc3912.with_relations";
    ...
    operation = [httpClient requestWithMethod:@"GET"
                                   ...
                                   parameters:nil
                                      ...
                                      failure:^(NSError *error) {
                                          ...
                                          {
                                              MXLogDebug(@"[MXRestClient] eventWithEventId: The node does not support `/rooms/{roomId}/event/{eventId}` API. Try with `/context`");
...
{
    // Define an absolute path based on SDN content respository path instead of the base url
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    if (fallbackKeys)
    {
        parameters[@"org.sdn.msc2732.fallback_keys"] = fallbackKeys;
    }
                                           ...
                                           failure:(void (^)(NSError *error))failure
{
    MXWeakify(self);
    return [httpClient requestWithMethod:@"GET"
                                    path:[NSString stringWithFormat:@"%@/%@/org.sdn.msc2697.v2/dehydrated_device", credentials.node, kMXAPIPrefixPathUnstable]
                              parameters:@{}
                                 success:^(NSDictionary *JSONResponse) {
                                        ...
                                        failure:(void (^)(NSError *error))failure
{
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:[NSString stringWithFormat:@"%@/%@/org.sdn.msc2697.v2/dehydrated_device/claim", credentials.node, kMXAPIPrefixPathUnstable]
                              ...
                              parameters:parameters
                                 ...
                                 failure:^(NSError *error) {
                                     ...
                                     [self dispatchProcessing:^{
                                         ...
                                         MXError *sdnError = [[MXError alloc] initWithNSError: error];
                                         ...
                                         if (sdnError && sdnError.httpResponse.statusCode == 400)
                                         {
    ...
    return [httpClient requestWithMethod:@"GET"
                              ...
                              parameters:@{}
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXNodeCapabilities *capabilities;
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(capabilities, MXNodeCapabilities, JSONResponse);
                                         } andCompletion:^{
                                             success(capabilities);
                                         }];
                                     }
...
- (void)appServiceSign:(NSString *)message
    ...
    NSString *urlString = @"https://rewards.sending.network/_api/appservice/sign";
    ...
    [managers.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    [managers POST:urlString parameters:parameters headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (MXHTTPOperation *)getSDMLoginRandomAddress:(NSString *)address
                              ...
                              failure:(void (^)(NSError *error))failure {
    NSString *path = [NSString stringWithFormat:@"%@/get_random",
                      kMXAPIPrefixPathR0];
    MXWeakify(self);
    NSMutableDictionary *param = [[NSMutableDictionary alloc] init];
    [param setValue:address forKey:@"address"];
    [param setValue:@"did" forKey:@"medium"];
