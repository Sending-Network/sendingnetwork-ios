...
/**
 ...
 It stores secrets stored locally (`MXCryptoStore`) on the node SSSS (`MXSecretStorage`)
 and vice versa.
 */
...
/**
 Indicates if a recovery/SSSS is set up on the node.
 */
- (BOOL)hasRecovery;
...
/**
 ...
 It will send keys from the local storage to the recovery on the node.
...
/**
 Restore keys from the recovery stored on the node to the local storage.
