...
@synthesize storeService, eventStreamToken, userAccountData, syncFilterId, nodeWellknown, areAllIdentityServerTermsAgreed;
@synthesize nodeCapabilities;
@synthesize supportedSDNVersions;
...
- (void)deleteAllMessagesInRoom:(NSString *)roomId
{
    ...
    roomStore.hasReachedNodePaginationEnd = NO;
}
...
{
    MXMemoryRoomStore *roomStore = [self getOrCreateRoomStore:roomId];
    roomStore.hasReachedNodePaginationEnd = value;
}

- (BOOL)hasReachedNodePaginationEndForRoom:(NSString*)roomId
{
    MXMemoryRoomStore *roomStore = [self getOrCreateRoomStore:roomId];
    return roomStore.hasReachedNodePaginationEnd;
}
...
{
    supportedSDNVersions = supportedSDNVersions;
}
