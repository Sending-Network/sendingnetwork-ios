...
NSString *const MXCrossSigningErrorDomain = @"org.sdn.sdk.crosssigning";
                  ...
                  failure:(void (^)(NSError *error))failure
{
    ...
    [self.crypto.sdnRestClient authSessionToUploadDeviceSigningKeys:^(MXAuthenticationSession *authSession) {
                    ...
                    failure:(void (^)(NSError *error))failure
{
    ...
    [self storeCrossSigningKeys:privateKeys success:^{
        ...
        [self.crypto.sdnRestClient uploadDeviceSigningKeys:signingKeys authParams:authParams success:^{
...
- (void)resetTrust
{
    MXLogDebug(@"[MXCrossSigning] resetTrust for device %@", self.crypto.mxSession.sdnRestClient.credentials.deviceId);
...
- (void)usersDevicesDidUpdate:(NSNotification*)notification
{
    ...
    MXCredentials *myUser = _crypto.mxSession.sdnRestClient.credentials;
           ...
           failure:(void (^)(NSError *error))failure
{
    ...
    [self signObject:object
             ...
             success:^(NSDictionary *signedObject)
     {
         ...
         [self.crypto.mxSession.sdnRestClient uploadKeySignatures:@{
                                                                       myUserId: @{
