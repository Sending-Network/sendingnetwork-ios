...
- (id)initWithRoomId:(NSString *)roomId andSDNSession:(MXSession *)mxSession2
{
    // Let's the live MXEventTimeline use its default store.
    return [self initWithRoomId:roomId sdnSession:mxSession2 andStore:nil];
}

- (id)initWithRoomId:(NSString *)roomId sdnSession:(MXSession *)mxSession2 andStore:(id<MXStore>)store
{
    self = [self init];
    if (self)
    {
...
+ (id)loadRoomFromStore:(id<MXStore>)store withRoomId:(NSString *)roomId sdnSession:(MXSession *)sdnSession
{
    MXRoom *room = [[MXRoom alloc] initWithRoomId:roomId andSDNSession:sdnSession];
    if (room)
    {
    ...
    if (needToLoadLiveTimeline || pendingLiveTimelineRequesters)
    {
        ...
        if (!pendingLiveTimelineRequesters)
        {
            ...
            [mxSession.store loadRoomMessagesForRoom:self.roomId completion:^{
                ...
                MXWeakify(self);
                [MXRoomState loadRoomStateFromStore:self.mxSession.store withRoomId:self.roomId sdnSession:self.mxSession onComplete:^(MXRoomState *roomState) {
                    MXStrongifyAndReturnIfNil(self);
    ...
    [self liveTimeline:^(id<MXEventTimeline> liveTimeline) {
        ...
        else
        {
            ...
            if (!self->pendingMembersRequesters)
            {
                ...
                MXWeakify(self);
                MXHTTPOperation *operation2 = [self.mxSession.sdnRestClient membersOfRoom:self.roomId
                             ...
                             failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sdnRestClient sendEventToRoom:self.roomId threadId:threadId eventType:eventTypeString content:newContent txnId:txnId success:^(NSString *eventId) {
...
- (MXHTTPOperation*)sendStateEventOfType:(MXEventTypeString)eventTypeString
                                ...
                                stateKey:(NSString *)stateKey
                                 ...
                                 failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient sendStateEventToRoom:self.roomId eventType:eventTypeString content:content stateKey:stateKey success:success failure:failure];
}
                      ...
                      failure:(void (^)(NSError *error))failure
{
    ...
    MXMediaLoader *uploader = [MXMediaManager prepareUploaderWithSDNSession:mxSession initialRange:0 andRange:endRange];
    NSString *fakeMediaURI = uploader.uploadId;
    
    NSString *cacheFilePath = [MXMediaManager cachePathForSDNContentURI:fakeMediaURI andType:mimetype inFolder:self.roomId];
    ...
    roomOperation = [self preserveOperationOrder:event block:^{
        ...
        if (self.mxSession.crypto && [self.mxSession.crypto isRoomEncrypted:self.summary.roomId])
        {
            ...
            [MXEncryptedAttachments encryptAttachment:uploader localUrl:localURL success:^(MXEncryptedContentFile *result) {
                ...
                else
                {
                    ...
                    MXMediaLoader *thumbUploader = [MXMediaManager prepareUploaderWithSDNSession:self.mxSession initialRange:0.9 andRange:1];
        ...
        {
            // Launch the upload to the SDN Content repository
            [uploader uploadData:imageData filename:filename mimeType:mimetype success:^(NSString *url) {
                ...
                NSString *actualCacheFilePath = [MXMediaManager cachePathForSDNContentURI:url andType:mimetype inFolder:self.roomId];
                NSError *error;
                [[NSFileManager defaultManager] copyItemAtPath:cacheFilePath toPath:actualCacheFilePath error:&error];

                // Update the message content with the mxc:// of the media on the node
                msgContent[@"url"] = url;
                           ...
                           failure:(void (^)(NSError *error))failure
{
    ...
    // directly from its cache
    // Pass this id in the URL is a nasty trick to retrieve it later
    MXMediaLoader *thumbUploader = [MXMediaManager prepareUploaderWithSDNSession:self.mxSession initialRange:0 andRange:0.1];
    NSString *fakeMediaURI = thumbUploader.uploadId;
    
    NSString *cacheFilePath = [MXMediaManager cachePathForSDNContentURI:fakeMediaURI andType:@"image/jpeg" inFolder:self.roomId];
        ...
        [MXTools convertVideoAssetToMP4:videoAsset
            ...
            if (self.mxSession.crypto && self.summary.isEncrypted)
            {
                [MXEncryptedAttachments encryptAttachment:thumbUploader data:videoThumbnailData success:^(MXEncryptedContentFile *result) {
                    ...
                    MXMediaLoader *videoUploader = [MXMediaManager prepareUploaderWithSDNSession:self.mxSession initialRange:0.1 andRange:1];
            ...
            else
            {
                ...
                [thumbUploader uploadData:videoThumbnailData filename:nil mimeType:@"image/jpeg" success:^(NSString *thumbnailUrl) {
                    ...
                    if (videoData)
                    {
                        ...
                        NSString *actualCacheFilePath = [MXMediaManager cachePathForSDNContentURI:thumbnailUrl andType:@"image/jpeg" inFolder:self.roomId];
                        NSError *error;
                        [[NSFileManager defaultManager] copyItemAtPath:cacheFilePath toPath:actualCacheFilePath error:&error];

                        MXMediaLoader *videoUploader = [MXMediaManager prepareUploaderWithSDNSession:self.mxSession initialRange:0.1 andRange:0.9];
                        ...
                        [videoUploader uploadData:videoData filename:filename mimeType:mimetype success:^(NSString *videoUrl) {
                            ...
                            NSString *actualCacheFilePath = [MXMediaManager cachePathForSDNContentURI:videoUrl andType:mimetype inFolder:self.roomId];
                            ...
                            // And send the SDN room message video event to the node (the sent state of the local echo will be updated, its local storage too).
                            MXHTTPOperation *operation2 = [self sendMessageWithContent:msgContent threadId:threadId localEcho:&event success:onSuccess failure:onFailure];
           ...
           keepActualFilename:(BOOL)keepActualName
{
    ...
    // directly from its cache
    // Pass this id in the URL is a nasty trick to retrieve it later
    MXMediaLoader *uploader = [MXMediaManager prepareUploaderWithSDNSession:self.mxSession initialRange:0 andRange:1];
    NSString *fakeMediaURI = uploader.uploadId;
...
- (MXHTTPOperation*)setTopic:(NSString*)topic
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomTopic:self.roomId topic:topic success:success failure:failure];
}
...
- (MXHTTPOperation*)setAvatar:(NSString*)avatar
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomAvatar:self.roomId avatar:avatar success:success failure:failure];
}
...
- (MXHTTPOperation*)setName:(NSString*)name
                    ...
                    failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomName:self.roomId name:name success:success failure:failure];
}
...
- (MXHTTPOperation *)setHistoryVisibility:(MXRoomHistoryVisibility)historyVisibility
                                  ...
                                  failure:(void (^)(NSError *))failure
{
    return [mxSession.sdnRestClient setRoomHistoryVisibility:self.roomId historyVisibility:historyVisibility success:success failure:failure];
}
...
- (MXHTTPOperation*)setJoinRule:(MXRoomJoinRule)joinRule
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomJoinRule:self.roomId joinRule:joinRule success:success failure:failure];
}
- (MXHTTPOperation*)setJoinRule:(MXRoomJoinRule)joinRule
                      parentIds:(NSArray<NSString *>*)parentIds
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomJoinRule:joinRule forRoomWithId:self.roomId allowedParentIds:parentIds success:success failure:failure];
}
...
- (MXHTTPOperation*)setGuestAccess:(MXRoomGuestAccess)guestAccess
                           ...
                           failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomGuestAccess:self.roomId guestAccess:guestAccess success:success failure:failure];
}
...
- (MXHTTPOperation*)setDirectoryVisibility:(MXRoomDirectoryVisibility)directoryVisibility
                                   ...
                                   failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomDirectoryVisibility:self.roomId directoryVisibility:directoryVisibility success:success failure:failure];
}
...
- (MXHTTPOperation*)addAlias:(NSString *)roomAlias
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient addRoomAlias:self.roomId alias:roomAlias success:success failure:failure];
}
...
- (MXHTTPOperation*)removeAlias:(NSString *)roomAlias
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient removeRoomAlias:roomAlias success:success failure:failure];
}
...
- (MXHTTPOperation*)setCanonicalAlias:(NSString *)canonicalAlias
                              ...
                              failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomCanonicalAlias:self.roomId canonicalAlias:canonicalAlias success:success failure:failure];
}

- (MXHTTPOperation*)directoryVisibility:(void (^)(MXRoomDirectoryVisibility directoryVisibility))success
                                failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient directoryVisibilityOfRoom:self.roomId success:success failure:failure];
}
                 ...
                 failure:(void (^)(NSError *error))failure
{
    ...
    // The user node already knows other nodes
...
- (MXHTTPOperation*)inviteUser:(NSString*)userId
                       ...
                       failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient inviteUser:userId toRoom:self.roomId success:success failure:failure];
}
...
- (MXHTTPOperation*)inviteUserByEmail:(NSString*)email
                              ...
                              failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient inviteUserByEmail:email toRoom:self.roomId success:success failure:failure];
}
...
- (MXHTTPOperation*)kickUser:(NSString*)userId
                     ...
                     failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient kickUser:userId fromRoom:self.roomId reason:reason success:success failure:failure];
}
...
- (MXHTTPOperation*)banUser:(NSString*)userId
                    ...
                    failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient banUser:userId inRoom:self.roomId reason:reason success:success failure:failure];
}
...
- (MXHTTPOperation*)unbanUser:(NSString*)userId
                      ...
                      failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient unbanUser:userId inRoom:self.roomId success:success failure:failure];
}
...
- (MXHTTPOperation*)sendTypingNotification:(BOOL)typing
                                   ...
                                   failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient sendTypingNotificationInRoom:self.roomId typing:typing timeout:timeout success:success failure:failure];
}
...
- (MXHTTPOperation*)redactEvent:(NSString*)eventId
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient redactEvent:eventId inRoom:self.roomId reason:reason success:success failure:failure];
}
...
- (MXHTTPOperation*)redactEvent:(NSString*)eventId
                  withRelations:(NSArray<NSString *>*)relations
                        ...
                        failure:(void (^)(NSError *error))failure
{
    BOOL isFeatureWithRelationsSupported = (mxSession.store.supportedSDNVersions.supportsRedactionWithRelations || mxSession.store.supportedSDNVersions.supportsRedactionWithRelationsUnstable);
    BOOL isFeatureWithRelationsStable = mxSession.store.supportedSDNVersions.supportsRedactionWithRelations;
    ...
    return [mxSession.sdnRestClient redactEvent:eventId
                                            inRoom:self.roomId
                                            reason:reason
                                             txnId:nil
...
- (MXHTTPOperation*)setRelatedGroups:(NSArray<NSString *>*)relatedGroups
                        ...
                        failure:(void (^)(NSError *error))failure
{
    return [mxSession.sdnRestClient setRoomRelatedGroups:self.roomId relatedGroups:relatedGroups success:success failure:failure];
}
...
/**
 ...
 `<mx-reply><blockquote><a href=\"https://sdn.to/#/!vjFxDRtZSSdspfTSEr:sdn.org/$15237084491191492ssFoA:sdn.org\">In reply to</a> <a href=\"https://sdn.to/#/@sender:sdn.org\">@sender:sdn.org</a><br>sent an image.</blockquote></mx-reply>Reply message`
                   ...
                   failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sdnRestClient addTag:tag withOrder:order toRoom:self.roomId success:success failure:failure];
}
...
{
    // _accountData.tags will be updated by the live streams
    return [mxSession.sdnRestClient removeTag:tag fromRoom:self.roomId success:success failure:failure];
}
                     ...
                     failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sdnRestClient updateTaggedEvents:_roomId withContent:taggedEvents success:success failure:failure];
}
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    return [mxSession.sdnRestClient updateTaggedEvents:_roomId withContent:_accountData.taggedEvents success:success failure:failure];
}
...
- (MXHTTPOperation *)setAccountData:(NSDictionary *)content
                            ...
                            failure:(void (^)(NSError *))failure
{
    return [mxSession.sdnRestClient setRoomAccountData:_roomId
                                                eventType:type
...
- (void)acknowledgeEvent:(MXEvent*)event andUpdateReadMarker:(BOOL)updateReadMarker
{
    ...
    else if (updatedReadReceiptEvent)
    {
        ...
        [mxSession.sdnRestClient sendReadReceipt:self.roomId
...
- (void)setReadMarker:(NSString*)eventId withReadReceipt:(NSString*)receiptEventId threadId:(NSString*)threadId
{
    ...
    // Update data on the node side
    [mxSession.sdnRestClient sendReadMarker:self.roomId readMarkerEventId:eventId readReceiptEventId:nil success:nil failure:nil];
                                           ...
                                           success:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    ...
    if (mxSession.crypto)
    {
        // Send the information to the node
