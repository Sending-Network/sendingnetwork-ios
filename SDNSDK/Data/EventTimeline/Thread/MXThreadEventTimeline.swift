...
public class MXThreadEventTimeline: NSObject, MXEventTimeline {
    ...
    private var hasReachedNodeBackwardsPaginationEnd: Bool = false
    private var hasReachedNodeForwardsPaginationEnd: Bool = false
    ...
    public required init(thread: MXThread, initialEventId: String?, andStore store: MXStore) {
        ...
        if initialEventId == nil {
            ...
            hasReachedNodeBackwardsPaginationEnd = true
            hasReachedNodeForwardsPaginationEnd = true
        }
        self.store = store
        self.thread = thread
        self.state = MXRoomState(roomId: thread.roomId, andSDNSession: thread.session, andDirection: true)
        super.init()
    ...
    public func canPaginate(_ direction: MXTimelineDirection) -> Bool {
        ...
        case .backwards:
            ...
            return storeMessagesEnumerator?.remaining ?? 0 > 0 || !hasReachedNodeBackwardsPaginationEnd
        case .forwards:
            if isLiveTimeline {
                // SDN is not yet able to guess the future
                return false
            } else {
                return !hasReachedNodeForwardsPaginationEnd
            }
        @unknown default:
    ...
    public func resetPagination() {
        ...
        if isLiveTimeline {
            hasReachedNodeBackwardsPaginationEnd = false
            hasReachedNodeForwardsPaginationEnd = false
        }
    }
    ...
    public func __resetPaginationAroundInitialEvent(withLimit limit: UInt,
        ...
        //  This is due to https://github.com/Sending-Network/element-meta/issues/150
    ...
    public func __paginate(_ numItems: UInt, direction: MXTimelineDirection, onlyFromStore: Bool, complete: @escaping () -> Void, failure: @escaping (Error) -> Void) -> MXHTTPOperation {
        ...
        paginateFromStore(numberOfItems: numItems, direction: direction) { [weak self] eventsFromStore in
            ...
            if direction == .backwards {
                ...
                if remainingNumItems <= 0 || self.hasReachedNodeBackwardsPaginationEnd {
                    DispatchQueue.main.async {
                        // Nothing more to do
                        MXLog.debug("[MXThreadEventTimeline][\(self.timelineId)] paginate: is done from the store")
                        complete()
            ...
            if direction == .forwards && self.hasReachedNodeForwardsPaginationEnd {
                DispatchQueue.main.async {
                    // Nothing more to do
            ...
            if let sdnRestClient = self.thread.session?.sdnRestClient {
                let operation2 = sdnRestClient.relations(forEvent: self.thread.id,
        ...
        decryptEvents(response.chunk) {
            ...
            let updatePaginationInfo = {
                ...
                case .backwards:
                    ...
                    self.hasReachedNodeBackwardsPaginationEnd = response.nextBatch == nil
                    MXLog.debug("[MXThreadEventTimeline][\(self.timelineId)] processPaginationResponse: hasReachedNodeBackwardsPaginationEnd: \(self.hasReachedNodeBackwardsPaginationEnd)")
                case .forwards:
                    self.forwardsPaginationToken = response.nextBatch
                    self.hasReachedNodeForwardsPaginationEnd = response.nextBatch == nil
                    MXLog.debug("[MXThreadEventTimeline][\(self.timelineId)] processPaginationResponse: hasReachedNodeForwardsPaginationEnd: \(self.hasReachedNodeForwardsPaginationEnd)")
                @unknown default:
