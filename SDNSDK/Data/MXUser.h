...
/**
 `MXUser` represents a user in SDN.
 */
...
/**
 Make a request to the node to update the MXUser.

 @param mxSession the mxSession to the home server.
 @param success a block when the operation succeeds.
 @param failure a block when the operation fails.
 */
- (void)updateFromNodeOfSDNSession:(MXSession*)mxSession success:(void (^)(void))success failure:(void (^)(NSError *error))failure;
