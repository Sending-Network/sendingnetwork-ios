...
{
    // Need to check the current backup version on the node
    MXKeyBackupStateUnknown = 0,

    // Making the check request on the node
    ...
    // There is a backup available on the node but it is not trusted.
    ...
    // The backup version is being created on the node
    MXKeyBackupStateEnabling,

    // Backup is enabled and ready to send backup to the node
    MXKeyBackupStateReadyToBackUp,

    // Backup is going to be send to the node
    MXKeyBackupStateWillBackUp,

    // Backup is being sent to the node
...
/**
 Get information about a backup version defined on the node.
...
/**
 Check if the module uses the latest version available on the node.
...
/**
 Restore a backup with a recovery key from a given backup version stored on the node.
...
/**
 Restore a backup with a password from a given backup version stored on the node.
