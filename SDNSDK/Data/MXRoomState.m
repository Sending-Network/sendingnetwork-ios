...
@interface MXRoomState ()
{
    ...
    /**
     The third party invites. The key is the token provided by the node.
     */
...
- (id)initWithRoomId:(NSString*)roomId
    andSDNSession:(MXSession*)sdnSession
        andDirection:(BOOL)isLive
{
    ...
    if (self)
    {
        ...
        _members = [[MXRoomMembers alloc] initWithRoomState:self andSDNSession:mxSession];
...
- (id)initWithRoomId:(NSString*)roomId
    andSDNSession:(MXSession*)sdnSession
        ...
        andDirection:(BOOL)isLive
{
    self = [self initWithRoomId:roomId andSDNSession:sdnSession andDirection:isLive];
    if (self)
    {
...
+ (void)loadRoomStateFromStore:(id<MXStore>)store
               ...
               sdnSession:(MXSession *)sdnSession
                  onComplete:(void (^)(MXRoomState *roomState))onComplete
{
    ...
    MXRoomState *roomState = [[MXRoomState alloc] initWithRoomId:roomId andSDNSession:sdnSession andDirection:YES];
    if (roomState)
    {
        [store stateOfRoom:roomId success:^(NSArray<MXEvent *> * _Nonnull stateEvents) {
            if (!stateEvents.count) {
                ...
                [sdnSession.sdnRestClient stateOfRoom:roomId success:^(NSArray *JSONData) {
