...
/**
 ...
 SDN events types are exchanged as strings with the home server. The types
 specified by the SDN standard are listed here as NSUInteger enum in order
...
/**
 Types of SDN events - String version
 The event types as described by the SDN standard.
 */
...
/**
 The string event (decrypted, if necessary) type as provided by the node.
...
/**
 ...
 Check http://sdn.org/docs/spec/client_server/r0.2.0.html#room-events to get a list of content keys per
 event type.
...
/**
 The string event (possibly encrypted) type as provided by the node.
...
/**
 The timestamp in ms since Epoch generated by the origin node when it receives the event
 from the client.
 */
@property (nonatomic) uint64_t originServerTs;

/**
 Information about this event which was not sent by the originating node.
