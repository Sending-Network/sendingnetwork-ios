...
public struct MXRoomListDataFilterOptions: Equatable {
    ...
    ///Flag to fetch and order rooms according room IDs stored in the `im.vectSending.setting.breadcrumbs` event within the user account data.
    ...
    ///   - onlyBreadcrumbs: flag to fetch and order rooms according room IDs stored in the `im.vectSending.setting.breadcrumbs` event within the user account data.
    ///   - query: search query
    ///   - space: active space
