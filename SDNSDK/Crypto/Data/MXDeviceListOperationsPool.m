...
- (void)doKeyDownloadForUsers:(NSArray<NSString *> *)users token:(NSString *)token complete:(void (^)(NSDictionary<NSString *, NSDictionary *> *failedUserIds))complete
{
    ...
    // Download
    MXWeakify(self);
    _httpOperation = [crypto.sdnRestClient downloadKeysByChunkForUsers:users token:token chunkSize:kMXDeviceListOperationsPoolKeyQueryLimit success:^(MXKeysQueryResponse *keysQueryResponse) {
        ...
        // Delay
        dispatch_async(self->crypto.sdnRestClient.completionQueue, ^{
