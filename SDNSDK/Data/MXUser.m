...
- (void)updateWithRoomMemberEvent:(MXEvent*)roomMemberEvent roomMember:(MXRoomMember *)roomMember inSDNSession:(MXSession *)mxSession
{
...
- (void)updateWithPresenceEvent:(MXEvent*)presenceEvent inSDNSession:(MXSession *)mxSession
{
    ...
    // Note: It is about to change in a short future in SDN spec.
...
- (void)updateFromNodeOfSDNSession:(MXSession *)mxSession success:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    MXWeakify(self);
    [mxSession.sdnRestClient displayNameForUser:_userId success:^(NSString *displayname) {
        MXStrongifyAndReturnIfNil(self);

        MXWeakify(self);
        [mxSession.sdnRestClient avatarUrlForUser:self.userId success:^(NSString *avatarUrl) {
            MXStrongifyAndReturnIfNil(self);
            ...
            MXLogDebug(@"[MXUser] updateFromNodeOfSDNSession failed to get avatar");
            failure(error);
        }];
    } failure:^(NSError *error) {

        MXLogDebug(@"[MXUser] updateFromNodeOfSDNSession failed to get display name");
        failure(error);
    }];
}
