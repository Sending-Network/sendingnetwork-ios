...
@property (nonatomic, strong) MXRestClient *nodeRestClient;
...
- (instancetype)initWithIdentityServerRestClient:(MXIdentityServerRestClient*)identityServerRestClient andNodeRestClient:(MXRestClient*)nodeRestClient
{
    self = [super init];
    if (self)
    {
        ...
        self.nodeRestClient = nodeRestClient;
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleHTTPClientError:) name:kMXHTTPClientSDNErrorNotification object:nil];
...
- (MXHTTPOperation*)signUrl:(NSString*)signUrl
                    ...
                    failure:(void (^)(NSError *error))failure
{    
    return [self.restClient signUrl:signUrl mxid:self.nodeRestClient.credentials.userId success:success failure:failure];
}
...
- (MXHTTPOperation*)renewAccessTokenWithSuccess:(void (^)(NSString*))success failure:(void (^)(NSError *))failure
{
    ...
    operation = [self.nodeRestClient openIdToken:^(MXOpenIdToken *tokenObject) {
...
- (void)handleHTTPClientError:(NSNotification*)nofitication
{
    ...
    MXError *mxError;
    MXJSONModelSet(mxError, [MXError class], nofitication.userInfo[kMXHTTPClientSDNErrorNotificationErrorKey]);
...
- (NSDictionary*)notificationUserInfoWithAccessToken:(nonnull NSString*)accessToken
{
    ...
    NSString *userId = self.nodeRestClient.credentials.userId;
