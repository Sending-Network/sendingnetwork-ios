...
NSString *const MXCryptoMigrationErrorDomain = @"org.sdn.sdk.crypto.migration";
...
- (void)migrateToCryptoVersion2:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    ...
    // https://github.com/Sending-Network/sendingnetwork-ios/issues/3818
    MXWeakify(self);
...
- (void)claimOwnOneTimeKeys:(NSUInteger)keyCount success:(void (^)(NSUInteger keyClaimed))success failure:(void (^)(NSError *))failure
{
    ...
    for (NSUInteger i = 0; i < keyCount; i++)
    {
        ...
        [crypto.sdnRestClient claimOneTimeKeysForUsersDevices:usersDevicesToClaim success:^(MXKeysClaimResponse *keysClaimResponse) {
