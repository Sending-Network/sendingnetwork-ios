...
- (instancetype)initWithRoomState:(MXRoomState *)roomState andSDNSession:(MXSession*)sdnSession
{
    self = [super init];
    if (self)
    {
            ...
            switch (event.eventType)
            {
                case MXEventTypeRoomMember:
                {
                    ...
                    if (isLive && !event.isRedactedEvent && roomMember.membership == MXMembershipJoin)
                    {
                        ...
                        [user updateWithRoomMemberEvent:event roomMember:roomMember inSDNSession:mxSession];
