...
class MXCryptoMachine {
    ...
    private let processingQueue = DispatchQueue(label: "org.sdn.sdk.MXCryptoMachine.processingQueue")
    ...
    private static func createMachine(userId: String, deviceId: String, log: MXNamedLog) throws -> OlmMachine {
        ...
        log.debug("Opening crypto store at \(url.path)/sdn-sdk-crypto.sqlite3") // Hardcoding full path to db for debugging purposes
