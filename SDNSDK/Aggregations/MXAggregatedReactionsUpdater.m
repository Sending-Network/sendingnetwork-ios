...
- (instancetype)initWithSDNSession:(MXSession *)mxSession aggregationStore:(id<MXAggregationsStore>)store
{
    self = [super init];
    if (self)
    {
...
- (void)handleReaction:(MXEvent *)event direction:(MXTimelineDirection)direction
{
    ...
    if (parentEventId && reaction)
    {
        ...
        MXEvent *parentEvent = [self.sdnStore eventWithEventId:parentEventId inRoom:event.roomId];
        if (parentEvent)
        {
...
- (void)updateReactionCountForReaction:(NSString*)reaction forEvent:(NSString*)eventId reactionEvent:(MXEvent *)reactionEvent
{
    ...
    // Migrate data from sdn store to aggregation store if needed
...
/**
 ...
 @param block block called when the operation on the reaction can be sent to the node.
 */
...
/**
 Called when we get an acknowledgement by the node that our user has done
...
- (nullable NSArray<MXReactionCount*> *)reactionCountsOnEvent:(NSString*)eventId inRoom:(NSString*)roomId
{
    ...
    for (MXReactionRelation *relation in relations)
    {
        ...
        {
            // Have the same behavior as reactionCountsFromSDNStoreOnEvent
        ...
        if (!reactionCount.myUserReactionEventId)
        {
            ...
            MXEvent *event = [self.sdnStore eventWithEventId:relation.reactionEventId inRoom:roomId];
            if ([event.sender isEqualToString:self.myUserId])
            {
