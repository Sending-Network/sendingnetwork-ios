...
typedef NS_ENUM(NSUInteger, MXSessionState)
{
    ...
    /**
     The connection to the node is temporary lost.
     
     @discussion
     The SDN session will automatically establish it again. Once back, the state will move to
     MXSessionStateRunning.
     */
    MXSessionStateNodeNotReachable,

    /**
     The node returned a temporary error when trying to sync.
     Check `MXSession.syncError` to get this error.

     @discussion
     The SDN session will automatically go back MXSessionStateRunning once possible.
     */
    ...
    /**
     ...
     @discussion
     The SDN session will stay in this state until a new call of [MXSession start:failure:].
     */
...
/**
 Posted when the `directRooms` property is updated from node.
...
/**
 Posted when the virtual rooms are updated, either from the store or from the node.
...
/**
 Posted when the sdn account data are updated from node.
...
/**
 Posted when MXSession has updated the publicised groups of some sdn users.
...
/**
 The key in notification userInfo dictionary representating the sdn node
...
@class MXNodeCapabilitiesService;
 ...
 */
@property (nonatomic, readonly) MXRestClient *sdnRestClient;

/**
 The identity service used to handle SDN identity server requests. Can be nil.
 */
...
/**
 The media manager used to handle the media stored on the SDN Content repository.
 */
...
/**
 The user's sdn account data.
 */
@property (nonatomic, readonly) MXAccountData *accountData;

/**
 The store used to store user's SDN data.
 */
...
/**
 Capabilities of the current node
 */
@property (nonatomic, readonly) MXNodeCapabilitiesService *nodeCapabilitiesService;
...
/**
 ...
 However, the SDK or the application may want to continue so sync SDN events while
 the app is in background.

 This method prevents the /sync from being paused so that the session continues to receive
 and process SDN events.
...
#pragma mark - Options
/*
 Define the SDN storage component to use.
...
/**
 ...
 @return a MXHTTPOperation instance.
 */
- (MXHTTPOperation*)supportedSDNVersions:(void (^)(MXSDNVersions *sdnVersions))success failure:(void (^)(NSError *error))failure NS_REFINED_FOR_SWIFT;
...
/**
 Update the direct rooms list on node side.
...
/**
 ...
 This may lead to pagination requests to the node. Updated room summaries will be
...
/**
 ...
 Calling this method will paginate a maximum of 50 messages from the node.
...
/**
 ...
 @param maxServerPaginationCount the maximal number of messages to paginate from the node.
 */
...
/**
 ...
 @return an array of user ids. nil if the list has not been yet fetched from the node.
 */
...
/**
 The node .well-known data.
 */
@property (nonatomic, readonly) MXWellKnown *nodeWellknown;

/**
 Refresh self.nodeWellknown.
 ...
 @return a MXHTTPOperation instance.
 */
- (MXHTTPOperation*)refreshNodeWellknown:(void (^)(MXWellKnown *nodeWellknown))success
                                       failure:(void (^)(NSError *error))failure;
 ...
 */
@property (nonatomic, readonly) MXCapabilities *nodeCapabilities NS_REFINED_FOR_SWIFT;
...
/**
 Retrieve a SDN filter either from the store of from the node.

 @param filterId the id of the SDN filter to retrieve.
...
/**
 Force the sdn session to refresh the cached data related to the publicised groups of the users.
 */
- (void)markOutdatedPublicisedGroupsByUserData;

/**
 Get the current publicised groups for a sdn user.
