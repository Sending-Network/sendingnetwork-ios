...
/// Number of events retrieved when doing pagination from the node.
...
/**
 ...
 @return the new instance.
 */
- (instancetype)initWithRoomId:(NSString *)roomId sdnSession:(MXSession *)mxSession andStore:(id<MXStore>)store;
...
/**
 The SDN content URI of the room avatar.
 */
...
/**
 Reset the last message by paginating messages from the node if needed.
...
// @TODO(summary): paginationToken, hasReachedNodePaginationEnd, etc
...
/**
 ...
 @param session the session the room belongs to.
 @param summary the room summary.
 @param serverRoomSummary the node side room summary.
 @param roomState the current state of the room.
 @return YES if the room summary has changed.
 */
