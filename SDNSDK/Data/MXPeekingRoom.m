...
- (id)initWithRoomId:(NSString *)roomId andSDNSession:(MXSession *)mxSession
{
    ...
    [memoryStore openWithCredentials:mxSession.sdnRestClient.credentials onComplete:nil failure:nil];

    // Same thing for the summary
    summary = [[MXPeekingRoomSummary alloc] initWithRoomId:roomId sdnSession:mxSession andStore:memoryStore];
    [summary setPeekingRoom:self];

    return [self initWithRoomId:roomId sdnSession:mxSession andStore:memoryStore];
}
...
- (void)start:(void (^)(void))onServerSyncDone failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    httpOperation = [self.mxSession.sdnRestClient initialSyncOfRoom:self.roomId withLimit:0 success:^(MXRoomInitialSync *roomInitialSync) {
